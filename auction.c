...
void do_auction( CHAR_DATA *ch, char * argument )
{
	...
	{
	    sprintf(buf, "You must bid above the current bid of %ld Bones.\n\r",
	...
	sprintf(buf, "#B%s #wjust bid #Y%ld #wbones for %s.#n\n\r", ch->pcdata->switchname,
...
/*    if ( obj->questowner != NULL && strlen(obj->questowner) > 1 )
    ...
    sprintf(buf,"%s has put up %s for auction. [Minimum Bid: %ld bones]\n\r", ch->pcdata->switchname, obj->short_descr, auction_info.minbid);
        do_auchn( ch,buf );
...
void auction_update( )
{
    ...
    {
	sprintf(buf,"#w%s #R#fSOLD#n #wto %s for #Y%ld #0Bones.#n\n\r",
	    capitalize(auction_info.item->short_descr),
	...
	sprintf(temp,  "%ld bones ", auction_info.bones_held );
	sprintf(buf, "You receive %ld.\n\r",
    ...
    {
	sprintf(buf, "%s - going twice at %ld bones.\n\r",
		capitalize(auction_info.item->short_descr),
		auction_info.current_bid );
        do_auchn( auction_info.high_bidder,buf );
	return;
	...
	{
	    sprintf(buf, "No bids on %s - item sent back to Auctioner.\n\r",
		auction_info.item->short_descr);
            do_auchn( auction_info.high_bidder,buf );
	...
	send_to_char( buf, auction_info.owner );
        ...
        sprintf(buf, "%s - going once at %ld bones.\n\r",
                capitalize(auction_info.item->short_descr),
                auction_info.current_bid );
        do_auchn( auction_info.high_bidder,buf );
        return;
...
void show_obj_stats( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    send_to_char(buf,ch);
    sprintf( buf, "Weight is %d, value at %d.\n\r",
        obj->weight,
        obj->cost );
    send_to_char(buf,ch);
    ...
    sprintf( buf, capitalize(obj->short_descr));
