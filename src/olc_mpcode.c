...
OPEDIT (opedit_create)
{
    ...
    sprintf(buf, "ObjProgram Code Created for vnum %d", value);
    send_to_char(buf,ch);
...
MPEDIT( mpedit_list )
{
	...
	if ( fAll || ENTRE(ch->in_room->area->lvnum, mprg->vnum, ch->in_room->area->uvnum) )
	{
...
OPEDIT( opedit_list )
{
	...
	if ( fAll || ENTRE(ch->in_room->area->lvnum, oprg->vnum, ch->in_room->area->uvnum) )
	{
...
RPEDIT( rpedit_list )
{
	...
	if ( fAll || ENTRE(ch->in_room->area->lvnum, rprg->vnum, ch->in_room->area->uvnum) )
	{
