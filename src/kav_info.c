...
void do_scan( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char("#0[#WNorth#0]#n\n\r",ch);
    do_spydirection(ch,"n");
    char_from_room(ch);
    char_to_room(ch,location);

    send_to_char("#0[#WEast#0]#n\n\r",ch);
    do_spydirection(ch,"e");
    char_from_room(ch);
    char_to_room(ch,location);

    send_to_char("#0[#WSouth#0]#n\n\r",ch);
    do_spydirection(ch,"s");
    char_from_room(ch);
    char_to_room(ch,location);

    send_to_char("#0[#WWest#0]#n\n\r",ch);
    do_spydirection(ch,"w");
    char_from_room(ch);
    char_to_room(ch,location);

    send_to_char("#0[#WUp#0]#n\n\r",ch);
    do_spydirection(ch,"u");
    char_from_room(ch);
    char_to_room(ch,location);

    send_to_char("#0[#WDown#0]#n\n\r",ch);
...
void birth_date( CHAR_DATA *ch, bool is_self )
{
	...
	sprintf(buf,"  You are #C%d #nyears #C%d#n %s and #C%d #n%s old\n\r",
	...
	if (yy > 0 || (dd > 0 && mm >= 9) || IS_EXTRA(ch, EXTRA_LABOUR))
	{
	    ...
	    sprintf(buf,"#RYou are ready to give birth.#n\n\r");
	    send_to_char(buf,ch);
	}
	else if (yy > 0 || mm > 0)
	{
