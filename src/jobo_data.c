...
void update_top_board( CHAR_DATA *ch )
{
  ...
  for (i = 1; i <= MAX_TOP_PLAYERS && !changed; i++)
  {
    if (ch->race > top_board[i].pkscore)
    {
      ...
      top_board[i].name = str_dup(ch->pcdata->switchname);
      changed = TRUE;
      i = MAX_TOP_PLAYERS+1;
    }
  }
...
void check_leaderboard( CHAR_DATA *ch )
{
  ...
  if (IS_NPC(ch)) return;
  if (ch->level > 6) return;
  if (ch->race > leader_board.bestpk_number || !str_cmp(leader_board.bestpk_name , ch->name))
  {
    leader_board.bestpk_number = ch->race;
