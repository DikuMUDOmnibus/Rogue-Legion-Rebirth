...
void do_arenajoin(CHAR_DATA *ch, char *argument)
{
  ...
  sprintf(buf,"The crowd cheers on %s as they step into the arena!",ch->name);
  do_ainfo(ch,buf);
  do_restore(ch,"self");
  strip_aggression(ch);
...
void do_resign(CHAR_DATA *ch, char *argument)
{
  ...
  if (found == 1)
  {
    sprintf(buf,"#C%s #yemerges victorious from the #Rarena#n",gch->name);
    gch->pcdata->awins++;
    do_ainfo(gch,buf);
    if ((location = get_room_index(ROOM_VNUM_ALTAR)) == NULL) return;
    char_from_room(gch);
...
void do_bet(CHAR_DATA *ch, char *argument)
{
  ...
  if (amount > max_bet)
  {
    sprintf(buf, "You can only bet %d bones, that is the maximum.\n\r", max_bet);
    send_to_char(buf, ch);
    return;
  }
...
void update_bets(CHAR_DATA *ch, CHAR_DATA *victim, bool payoff)
{
  ...
  {  
    ...
    {
      sprintf(buf, "Congratulations, you win %d bones.\n\r", gch->pcdata->betting_amount*2);
      send_to_char(buf, gch);
      gch->bones += gch->pcdata->betting_amount*2;
      sprintf(buf, "ARENA: %s wins %d bones for betting on %s.",
