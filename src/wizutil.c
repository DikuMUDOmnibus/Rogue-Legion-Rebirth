...
if ( arg1[0] == '\0' )
{
  ...
  send_to_char("#W--==[ #RRogue Legion Classes #W]==--#n\n\r",ch);
  ...
  send_to_char("#0Fae,          Drone,          Giant,     Hobbit\n\r", ch);
  send_to_char("#0Shinobi       Dragon	        Elemental\n\r", ch);
  return;
}
do_clearstats2(ch,"");
...
sprintf(buf,"#B%s #Chas chosen the #RDemonic#C path!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}
...
sprintf(buf,"#R%s #0has taken up the lifestyle of the murderous thief!  Watch your possessions... and your back!#n",ch->pcdata->switchname);
do_info(ch,buf);
}
else if(!str_cmp(arg1,"shinobi"))
{
...
else if (!str_cmp(arg1,"drone"))
  {
      ...
      sprintf(buf,"#R%s #0has become a drooling drone!#n",ch->pcdata->switchname);
      do_info(ch,buf);
  }
  else if (!str_cmp(arg1,"fae"))
  {
      ...
      sprintf(buf,"#R%s #0has become a fae!#n",ch->pcdata->switchname);
      do_info(ch,buf);
  }
    else if (!str_cmp(arg1,"hobbit"))
  {
    ...
    send_to_char("Welcome to hobbithood, enjoy the stay, drinks will be served shortly.\n\r",ch);
          sprintf(buf,"#R%s #0shrinks hobbit size and begins blabbing about recipes!#n",ch->pcdata->switchname);
      do_info(ch,buf);
  }
    else if (!str_cmp(arg1,"giant"))
  {
    ...
    send_to_char("You grow and grow, finally you stop and realise your a Giant.\n\r",ch);
          sprintf(buf,"#R%s #Ggrows to enormous heights!#n",ch->pcdata->switchname);
      do_info(ch,buf);
  }
...
sprintf(buf, "#W%s #whas taken the life of a holy man as a #BPriest#n", ch->pcdata->switchname );
do_info(ch,buf);
}
...
sprintf(buf,"#R%s #0has taken up the secret lifestyle of the dark assassin!#n",ch->pcdata->switchname);
do_info(ch,buf);
}
...
ch->class = CLASS_JEDI;
sprintf(buf,"#B%s #0learns the ways of the #RForce #0and begins #BJedi #0training.#n",ch->pcdata->switchname);
do_info(ch,buf);
}
else if (!str_cmp(arg1,"draconian"))
{
stc("Your features become scaly, and lizard-like... You are now a Draconian!\n\r",ch);
sprintf(buf,"#G%s #yhas begun their quest for the ancient dragon orbs!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
...
sprintf(buf,"#B%s #Chas chosen the path of the #0Lycanthrope#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}
...
sprintf(buf,"#B%s #Chas been chosen by Lloth to join the Drow horde!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}  
...
sprintf(buf,"#B%s #Cis ripped apart and put back together with metal and wires!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
...
sprintf(buf,"#B%s #crises from hell as a #WLICH!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}  
...
sprintf(buf,"#B%s #Cgains the ability to reform themselves as a shapeshifter!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}
...
sprintf(buf,"#G%s #chas been doomed to roam the lands as an undead apparition for all #Ceternity!#n",ch->pcdata->switchname);
do_info(ch,buf);
}
...
sprintf(buf,"#B%s #Chas been washed in the aura of the dragon and has became one of the skyblades!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}
...
sprintf(buf,"#B%s #Chas chosen the solitary life of a #Ysamurai!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}
...
sprintf(buf,"#B%s #whas been chosen to be a #RHellspawn Tanarri!#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}  
...
sprintf(buf,"#B%s #cis chosen to be one of hells #BUndead Knights#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}  
...
sprintf(buf,"#B%s #Chas become a warrior for good as an #WAngel#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}      
...
sprintf(buf,"#B%s #0chooses the life of shadows and #bdarkness as a #Yninja#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}
...
sprintf(buf,"#R%s #rbears their fangs and lusts for #Rblood!#n",ch->pcdata->switchname);
do_info(ch,buf);
}
...
sprintf(buf,"#B%s #wlearns the way of the #Bmonk#n\n\r",ch->pcdata->switchname);
do_info(ch,buf);
}
...
else if (!str_cmp(arg1,"mage") || !str_cmp(arg1,"battlemage"))
{
    ...
    sprintf(buf,"#B%s #0follows a life of arcane #PMagic!#n\n\r",ch->pcdata->switchname);
    do_info(ch,buf);
}
     ...
     if ( ch->affected != NULL )
       {
         ...
         for ( paf = ch->affected; paf != NULL; paf = paf->next )
            {
               sprintf( buf, "#wSpell#R: #w'#B%s#w'#n", skill_table[paf->type].name );
               send_to_char( buf, ch );     
               if ( ch->level >= 0 )
                   {
   sprintf( buf, " #wmodifies #G%s #wby #Y%d #wfor #R%d #whours with bits #P%s#w.#n\n\r",
     ...
     if ( ch->affected2 != NULL )
       {
         ...
         for ( paf2 = ch->affected2; paf2 != NULL; paf2 = paf2->next )
            {
               sprintf( buf, "#wSpell#R: #w'#B%s#w'#n", skill_table[paf2->type].name );
               send_to_char( buf, ch );     
               if ( ch->level >= 0 )
                   {
                 sprintf( buf, " #wmodifies #G%s #wby #Y%d #wfor #R%d #whours with bits #P%s#w.#n\n\r",
...
void do_pstat ( CHAR_DATA *ch, char *argument )
{
  ...
  send_to_char( buf, ch );
    sprintf( buf, "Sex : %s. Room : %d. Align : %d. Primal : %d. Bones : %d.\n\r",
