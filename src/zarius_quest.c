...
void do_mobquest( CHAR_DATA *ch, char *argument )
{
   ...
   if ( !strcmp( argument, "stop" ) )
   {
     if ( quest )
     {
       sprintf( buf, "The quest has been stopped by an Immortal. Please speak up if you have already gotten the item.\n\r" );
       do_qtalk( ch, buf );
       clear_quest();
     }
     return;
...
void quest_inform( void )
{
   ...
   if ( quest_timer < 7 )
   {
    sprintf( buf, qmessages[quest_personality][quest_timer].message1, aquest_object->short_descr );
   }
   else
   { 
   if (quest_target)
   ...
   if ( quest_timer == 1 )
   {
     sprintf( buf, " %s is questing for %s ", NAME( quest_mob ), aquest_object->short_descr);
...
void quest_complete( CHAR_DATA *ch )
{
   ...
   sprintf( buf, qmessages[quest_personality][16].message1, NAME(ch), aquest_object->short_descr );
   do_qtalk( quest_mob, buf );
   clear_quest();
   return;
}   
...
void ask_quest_question( CHAR_DATA *ch, char *argument )
{
     ...
     || ( !str_cmp( argument, "who stole the item?" )		)  )
  {
    ...
    if ( aquest_object )
    {
      sprintf( buf, "Don't worry about who stole my %s, they have recieved their just reward!",
        aquest_object->short_descr );
    }
    if ( quest_mob != NULL )
      do_qtalk( quest_mob, buf );
  ...
  if ( !str_cmp( argument, "what item?" ) )
  {
    ...
    {
      sprintf(buf, "My %s was stolen from me, find it!.", aquest_object->short_descr );
      do_qtalk( quest_mob, buf );
      return;
    }
  }
  ...
  if ( !str_cmp( argument, "where are you?" ) )
    if ( quest_mob)
    {
      sprintf( buf, "You can find me in %s, please hurry!!", quest_mob->in_room->area->name);
      do_qtalk( quest_mob, buf );
      return;
    }
  ...
  if  ( !str_cmp( argument, "where is the thief?" ) )
  {
    if ( quest_mob )
    {
      ...
      {  
        if ( quest_timer < 10 )
        {
          sprintf( buf, "I don't really know where %s is, let me try and find out.", 
      ...
      {
        sprintf( buf, "Don't worry about where the thief who stole my %s is, they have recieved their just reward",
          aquest_object->short_descr );
      }
      do_qtalk( quest_mob, buf );
    }
