...
void do_tide( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->practice < 10)
    {
     send_to_char("You require at least 10 practice to use Tide of Vitae.\n\r",ch);
     return;
    } 
...
void do_forget( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->practice < 25)
    {
    send_to_char("You need 25 practice to use Forgetful Mind.\n\r", ch);
    return;
    }
...
void do_vampirearmor( CHAR_DATA *ch, char *argument )
{
  ...
  if (arg[0] == '\0')
  {
    ...
    if ( ch->practice < 60 )
    {
        send_to_char("It costs 60 points of practice to create vampire equipment.\n\r",ch);
        return;
    }
...
if (ch->embracing != NULL)
{
    ...
    if ((victim = get_char_room( ch, NULL, arg)) == NULL && ch->embracing==NULL)
    {
...
if (!str_cmp(arg,"u")) obj=create_object(get_obj_index(99),0);
obj_to_room(obj,ch->in_room);
obj->timer=3;
...
void do_assassinate( CHAR_DATA *ch, char *argument )
{
    ...
    return;
     ...
     if (IS_CLASS(ch, CLASS_SHADOW) && !IS_SET(ch->pcdata->powers[SHADOW_POWERS], NSHADOWS_ASSASSINATE))
  {
...
void do_bloodrod( CHAR_DATA *ch, char *argument )
{
    ...
    if ( 10 > ch->practice)
    {
        send_to_char("It costs 10 points of practice to create a Blood Rod.\n\r",ch);
        return;
    }
        ch->practice   -= 10;
...
{         
   ...
   if ( ( victim = get_char_room( ch, NULL, arg ) ) == NULL && (ch->fighting) == NULL)
    {
        send_to_char( "They aren't here.\n\r", ch );
        return;
    }
