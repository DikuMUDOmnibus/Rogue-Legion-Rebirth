 ...
 if ( ch->fighting != victim || dt == gsn_backstab || dt == gsn_shiroken
  ...
  if ( IS_CLASS(ch, CLASS_DEMON) && IS_SET(ch->warp, WARP_SPIKETAIL) && number_range(1,3) == 1)
  {
	...
	if (IS_CLASS(ch, CLASS_GHOUL) && ch->pcdata->powers[GPOWER_ZOMBIE] >= 2)
	{
		...
		if (rand_num > 2)	
	{
		sprintf(buf1,"#G$N #ccringes in pain as your diseased claws rend their flesh!  #g[#G%d#g]#n", dam);
		sprintf(buf2,"#cYou cringe in pain as #G$n's #cdiseased claws rend your flesh!  #g[#G%d#g]#n", dam);
  ...
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_ELEMENTAL) && ch->pcdata->powers[ELEMENTAL_FIRE] >=1 && number_range(1,4) == 1)
    multi_hit(ch, victim, gsn_fblast);
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_ELEMENTAL) && ch->pcdata->powers[ELEMENTAL_AIR] >=1 && number_range(1,4) == 1)
    multi_hit(ch, victim, gsn_wgust);
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_ELEMENTAL) && ch->pcdata->powers[ELEMENTAL_WATER] >=1 && number_range(1,4) == 1)
    multi_hit(ch, victim, gsn_torrent);
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_ELEMENTAL) && ch->pcdata->powers[ELEMENTAL_EARTH] >=1 && number_range(1,4) == 1)
  ...
  if (IS_CLASS(victim, CLASS_DRACONIAN) && victim->pcdata->stats[DRACONIAN_SPIKES] > 0)
	...
	{
		sprintf(buf1,"#Y$N's #Gspikes come in contact with you, piercing your flesh!  #r[#R%d#r]#n", dam);
		sprintf(buf2,"#GYour spikes come in contact with #Y$n, #Gpiercing their flesh! #r[#R%d#r]#n", dam);
		act(buf1,ch,NULL,victim, TO_CHAR);
        act(buf2,ch,NULL,victim, TO_VICT);
   ...
   //Thief Critical Strike
	if (IS_CLASS(ch, CLASS_THIEF) && IS_SET(ch->pcdata->powers[1], THIEF_CRITICALSTRIKE) && wield1 != NULL && wield1->value[3] == 11)
	{
		...
		if (rand_num >= 5)	
	{
		sprintf(buf1,"#0You #Rdeftly lunge #0with your #7dagger #0and score a #rC#Rritical #rH#Rit#n #0on #r$N!  #r[#R%d#r]#n", dam);
		sprintf(buf2,"#r$n #Rdeftly lunges #0at you, #Rslicing #0you in a vital area and scoring a #rC#Rritical #rH#Rit! #r[#R%d#r]#n", dam);
		act(buf1,ch,NULL,victim, TO_CHAR);
                act(buf2,ch,NULL,victim, TO_VICT);	
		hurt_person(ch,victim,dam);
	}
  ...
  if (IS_CLASS(ch, CLASS_SKYBLADE) && ch->pcdata->powers[SKYBLADE_SKILLS] >= 4)
   ...
   if ( wield1 != NULL && (IS_SET(wield1->extra_flags, ITEM_ELECENCHANT) || wield1->pIndexData->vnum == 42721))
	{	
		...
		sprintf(buf1,"#0Your weapon #oS#yH#7OC#yK#oS #0$N!  #0[#R%d#0]#n", dam);
		sprintf(buf2,"#0$n's weapon #oS#yH#7OC#yK#oS #0you!  #0[#R%d#0]#n", dam);
		act(buf1,ch,NULL,victim, TO_CHAR);
        act(buf2,ch,NULL,victim, TO_VICT);
	...
	{	
		...
		sprintf(buf1,"#0Your weapon #rIN#RCI#yN#7E#yR#RAT#rES #0$N!  #0[#R%d#0]#n", dam);
		sprintf(buf2,"#0$n's weapon #rIN#RCI#yN#7E#yR#RAT#rES #0you!  #0[#R%d#0]#n", dam);
		act(buf1,ch,NULL,victim, TO_CHAR);
        act(buf2,ch,NULL,victim, TO_VICT);
	...
	{	
		...
		sprintf(buf1,"#0Your weapon #cF#CRE#7E#CZE#cS #0$N!  #0[#R%d#0]#n", dam);
		sprintf(buf2,"#0$n's weapon #cF#CRE#7E#CZE#cS #0you!  #0[#R%d#0]#n", dam);
		act(buf1,ch,NULL,victim, TO_CHAR);
        act(buf2,ch,NULL,victim, TO_VICT);
	...
	{	
		...
		sprintf(buf1,"#0Your weapon's #gp#Goiso#gn #0seeps into #R$N!  #0[#R%d#0]#n", dam);
		sprintf(buf2,"#R$n's #0weapon #co#C0#gP#GoI#ys#GOn#GS#C0#co #0you!  #0[#R%d#0]#n", dam);
		act(buf1,ch,NULL,victim, TO_CHAR);
        act(buf2,ch,NULL,victim, TO_VICT);
...
int number_attacks( CHAR_DATA *ch, CHAR_DATA *victim )
{
  ...
  if (IS_NPC(victim))
  {
	...
	if (IS_CLASS(ch, CLASS_ELEMENTAL)) count += 5;
        if (IS_CLASS(ch, CLASS_DRAGON)) count += 5;
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, int handtype)
{
  ...
  if (!IS_NPC(victim) && IS_SET(victim->symbioteaff, SYMBIOTE_DISPLACER))
  {
    int ddodge = number_range(1,50);
    if (ddodge == 10 || ddodge == 20 || ddodge == 30 || ddodge == 40 || ddodge == 50)
    {
      stc("#0You displacer beast illusion takes the hit#n.\n\r",victim);
      stc("#0Your opponent's displacer beast illusion takes the hit#n.\n\r",ch);
      dam = 0;
    }
  }
...
//        if (!IS_SET(ch->act, PLR_BRIEF2))
	act("$N is burned by your $p.", ch, wield, victim, TO_CHAR);
//        if (!IS_SET(victim->act, PLR_BRIEF2)) 
	act("You are burned by $n's $p.", ch, wield, victim, TO_VICT);
...
//        if (!IS_SET(ch->act, PLR_BRIEF2)) 
	act("Your $p freezes $N.", ch, wield, victim, TO_CHAR);
//        if (!IS_SET(victim->act, PLR_BRIEF2)) 
...
int up_dam(CHAR_DATA *ch, CHAR_DATA *victim, int dam)
{
  if (!IS_NPC(ch))
  {
    ...
    {
      if (ch->pcdata->powers[HOBBIT_COMBAT] > 4) dam *= 1.4;
    }
...
int cap_dam(CHAR_DATA *ch, CHAR_DATA *victim, int dam)
{
  ...
  if ( IS_SET(victim->in_room->shinobi, ROOM_SEARTH) && IS_CLASS(victim,CLASS_SHINOBI) && 
victim->pcdata->powers[S_ELEMENT] != S_WIND) dam *= 0.9;
...
void update_damcap( CHAR_DATA *ch, CHAR_DATA *victim )
{
  ...
  if (!IS_NPC(ch))
  {
    ...
    if (IS_CLASS(ch,CLASS_SHINOBI)) max_dam += 2400;
    if (IS_CLASS(ch, CLASS_ELEMENTAL)) max_dam += 2500;
    if (IS_CLASS(ch, CLASS_DRAGON)) max_dam += 2500;
...
void hurt_person( CHAR_DATA *ch, CHAR_DATA *victim, int dam )
{
  ...
  if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_ELEMENTAL) && !IS_CLASS(ch, CLASS_ELEMENTAL))
    elemental_enflame(ch,victim,dam); // check for enflame
  ...
  if ( victim->position == POS_DEAD )
  {
	...
	ch->symbiotepoints++;
      ...
      if ( IS_CLASS(ch, CLASS_GHOUL) )
	  {
	    ...
	    if(global_cp)
	    aurag *= 2;
	    sprintf(buf,"#GYou savagely devour the corpse! #Y[#W%d#Y]#n\n\r",aurag);
	    stc(buf,ch);
      ...
      if ( IS_CLASS(ch, CLASS_PRIEST) )
	  {
	    ...
	    if(global_cp)
	    aurag *= 2;
	    sprintf(buf,"#WYou pray for the salvation of the slain! #BGain #w%d #YFaith#n\n\r",aurag);
	    stc(buf,ch);
		ch->pcdata->stats[FAITH_PTS] += aurag;
	  }
	  
      if (IS_CLASS(ch, CLASS_DEMON) || IS_CLASS(ch, CLASS_GIANT)
      || IS_CLASS(ch, CLASS_DROW) || IS_CLASS(ch,CLASS_CYBORG) || IS_CLASS(ch, CLASS_TANARRI) || IS_CLASS(ch, CLASS_DROID))
      {
        ...
        else
        {
          ...
          if(global_cp)
          {
          sprintf(buf,"#R** #WDouble CP #R*** #BYou gain #0(#W%d#0)#B class points.#n\n\r", victim->level*4);
        }
        else
        {
        	sprintf(buf,"#BYou gain #0(#W%d#0)#B class points.#n\n\r", victim->level*2);
        }
    ...
    {
      sprintf(buf, "You recive a %d bones bounty, for killing %s.\n\r", victim->pcdata->bounty, victim->name);
      send_to_char(buf, ch);
...
/* ------------ This is the end for superstances, Jobo ------------------- */
  ...
  if (!IS_NPC(ch))
  {
    ...
    if (IS_CLASS(ch, CLASS_SHINOBI)) chance -= 25;
    if (IS_CLASS(ch, CLASS_ELEMENTAL)) chance -= 25;
    if (IS_CLASS(ch, CLASS_DRAGON)) chance -= 25;
  ...
  if (!IS_NPC(victim))
  {
    ...
    if (IS_CLASS(victim, CLASS_HOBBIT)) chance += victim->pcdata->powers[HOBBIT_COMBAT]*5;
    ...
    if (IS_CLASS(victim, CLASS_SHINOBI)) chance += 25;
    if (IS_CLASS(victim, CLASS_ELEMENTAL)) chance += 25;
    if (IS_CLASS(victim, CLASS_DRAGON)) chance += 25;
  ...
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_SHINOBI)) chance -= 25;
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_ELEMENTAL)) chance -= 25;
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_DRAGON)) chance -= 25;
  ...
  {
    chance -= 5;
    if (IS_SET(ch->pcdata->powers[DROID_IMPLANTS], IMPLANT_ARMWIRE)) chance -= 20;
  }
  if (!IS_NPC(victim) && IS_CLASS(victim,CLASS_DROID))
  {  
    chance += 5; 
  ...
  if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_SHINOBI)) chance += 25;
  if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_ELEMENTAL)) chance += 25;
  if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_DRAGON)) chance += 25;
...
/* ------------ This is the end for superstances, Jobo ------------------- */
  ...
  {
    chance -= 5;
    if (IS_SET(ch->pcdata->powers[DROID_IMPLANTS], IMPLANT_360VISION)) chance -= 25;
  }
  if (!IS_NPC(victim) && IS_CLASS(victim,CLASS_DROID))
  {
    chance += 5; 
    if (IS_SET(victim->pcdata->powers[DROID_IMPLANTS], IMPLANT_360VISION)) chance += 25;
  }
  if (!IS_NPC(ch) && IS_CLASS(ch,CLASS_CYBORG)) chance -= ch->pcdata->powers[CYBORG_LIMBS]*4;
  if (!IS_NPC(victim) && IS_CLASS(victim,CLASS_CYBORG)) chance += victim->pcdata->powers[CYBORG_LIMBS]*4;
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_GIANT)) chance -= ch->pcdata->rank*3;
  ...
  if (!IS_NPC(ch) && !IS_NPC(victim) && skychance != 101)
  {
    ...
    if (!IS_NPC(ch) && !IS_SET(ch->act, PLR_BRIEF3) && skychance != 101)
      {
      if (random_message == 1 || random_message == 2)
  ...
  for ( fch = char_list; fch != NULL; fch = fch->next )
  {
    if ( fch == ch || ( fBoth && fch->fighting == ch ) )
    {
      ...
      if (IS_SET(fch->act2,PLR_AUTOSTORE))
		if(fch->tells)
		{
		    sprintf( buf, "You have #R%d#n tells waiting.\n\r", fch->tells );
...
void raw_kill( CHAR_DATA *victim )
{
  ...
  if ( victim == quest_mob )
  {
      sprintf( buf, "Oh well, I guess the quest is over, since I am about to DIE!!!!!" );
      do_qtalk( victim, buf );
      quest_mob = NULL;
  }
...
//    if (IS_SET(victim->affected_by2, AFF2_EXPMOD))
    if (IS_AFFECTED(ch, AFF2_EXPMOD))
    {
      xp_modifier += 200;
      strcat(buf2," #C2X EXP#n");
    }
    if (global_exp)
    {
      xp_modifier += 200;
    ...
    gain_exp( gch, xp );
        ...
        if (wield3 != NULL)
        {
        livingweap_needed = (wield3->weapon_level * 525000);

              if (wield3->weapon_level < 100)
                {
              livingweap_exp = (xp * .1);
              xp -= (livingweap_exp);
              ...
              send_to_char("#7Your #Rweapon #7has gained a Level#n\n\r", gch);
                  ...
                  wield3->weapon_points++;
              }}
         }
        if (wield4 != NULL)
        {
        livingweap_needed = (wield4->weapon_level * 525000);

              if (wield4->weapon_level < 100)
                {
              livingweap_exp = (xp * .1);
              xp -= (livingweap_exp);
	...
	if (ch->genexp >= expneeded)
	{
		ch->genexp = 0;
		ch->generation -= 1;
		sprintf(buf,"#B%s #0has advanced a frickin #GGeneration!#n",ch->name);
		do_info(ch,buf);
		do_save(ch,"");
	}
    }
...
//  if (IS_SET(obj->spectype2, XITEM_LIGHTPLATE)) dam *= 0.5;
     ...
     (IS_NPC(victim) && IS_SET(victim->act, ACT_NOPARTS)))
  {
    ...
    if ( dt == TYPE_HIT && !IS_NPC(ch) && !IS_VAMPAFF(ch,VAM_CLAWS) )
    {
      if (dam == 0)
      {
        sprintf( buf1, "$n %s $N's %s [%d]", vp, loc, dam );
        sprintf( buf2, "You %s $N's %s [%d]", vs, loc, dam );
      ...
      else
      {
        sprintf( buf1, "$n %s $N's %s [%d]", vp, loc, dam );
        sprintf( buf2, "You %s $N's %s [%d]", vs, loc, dam );
        sprintf( buf3, "$n %s your %s [%d]", vp, loc, dam );
        critical = TRUE;
      }
    }
    else if ( dt == TYPE_HIT && !IS_NPC(ch) && IS_SET(ch->newbits, NEW_MONKFLAME))
    {
      if (dam >= 1)
      {
        if (!IS_CLASS(ch, CLASS_WEREWOLF))
        {
          sprintf( buf1, "$n's flaming hands %s $N's %s [%d]", vp, loc, dam );
          sprintf( buf2, "Your flaming hands %s $N's %s [%d]", vs, loc, dam );
          sprintf( buf3, "$n's flaming hands %s your %s [%d]", vp, loc, dam );
        }
        else
        {
          sprintf( buf1, "$n's flaming claws %s $N's %s [%d]", vp, loc, dam );
          sprintf( buf2, "Your flaming claws %s $N's %s [%d]", vs, loc, dam );
          sprintf( buf3, "$n's flaming claws %s your %s [%d]", vp, loc, dam );
        }
      }
      else 
      {
        sprintf( buf1, "$n's hit %s $N's %s [%d]", vp, loc, dam );
        sprintf( buf2, "Your hit %s $N's %s [%d]", vs, loc, dam );
        sprintf( buf3, "$n's hit %s your %s [%d]",   vp, loc, dam );
        critical = TRUE;
      }
    }
    else
    {
      ...
      if (dam == 0)
      {
        sprintf( buf1, "$n's %s %s $N's %s [%d]",  attack, vp, loc, dam );
        sprintf( buf2, "Your %s %s $N's %s [%d]",  attack, vp, loc, dam );
        sprintf( buf3, "$n's %s %s your %s [%d]",  attack, vp, loc, dam );
      }
      else if (IS_SET(ch->newbits, NEW_MONKFLAME) && (dam == 0))
      {
        sprintf( buf1, "$n's flaming %s %s $N's %s [%d]", attack2, vp, loc, dam );
        sprintf( buf2, "Your flaming %s %s $N's %s [%d]",  attack, vp, loc, dam );
        sprintf( buf3, "$n's flaming %s %s your %s [%d]", attack2, vp, loc, dam );
      }
      else
      {
        if ( dt >= 0 && dt < MAX_SKILL )
        {
          sprintf( buf1, "$n's %s %s $N's %s [%d]",  attack2, vp, loc, dam );
          sprintf( buf2, "Your %s %s $N's %s [%d]",   attack, vp, loc, dam );
          sprintf( buf3, "$n's %s %s your %s [%d]",  attack2, vp, loc, dam );
        }
        ...
        else
        {
          sprintf( buf1, "$n's %s %s $N's %s [%d]",  attack, vp, loc, dam );
          sprintf( buf2, "Your %s %s $N's %s [%d]",  attack, vp, loc, dam );
          sprintf( buf3, "The $n's %s %s your %s [%d]",   attack, vp, loc, dam );
          critical = TRUE;
        }
      }
    }
...
void trip( CHAR_DATA *ch, CHAR_DATA *victim )
{
  ...
  if ( victim->wait == 0)
  {
	...
	sprintf(buf,"$n's tail lash trips you, and you go down!");
    ADD_COLOUR(ch,buf,WHITE);
    act( buf, ch, NULL, victim, TO_VICT    );
    sprintf(buf,"Your tail lash trips $N and $E goes down!");
    ADD_COLOUR(ch,buf,WHITE);
    act( buf,  ch, NULL, victim, TO_CHAR    );
    sprintf(buf,"$n's tail lash trips $N and $E goes down!");
    ADD_COLOUR(ch,buf,WHITE);
    act( buf,  ch, NULL, victim, TO_NOTVICT );
    hurt_person(ch,victim,dam);
...
void do_berserk(CHAR_DATA *ch, char *argument)
{
  ...
  act("You go #7B#0E#7R#0S#7E#0R#7K#0!#n",ch,NULL,NULL,TO_CHAR);
  act("$n goes #7B#0E#7R#0S#7E#0R#7K#0!#n",ch,NULL,NULL,TO_ROOM);
...
void do_decapitate( CHAR_DATA *ch, char *argument )
{
  ...
  if (paradox_punish)
  {
    ...
    wiznet( ch, WIZ_DEATH, get_trust(ch), log_buf );
    log_string( log_buf );
...
void decap_message(CHAR_DATA *ch, CHAR_DATA *victim)
{
  ...
  if (IS_CLASS(ch, CLASS_JEDI))
			sprintf(buf,"#B%s #0drops to his knees and practically melts as %s's #Wlightsabre#0 Is thrust into the lazer charred corpse.#n", victim->pcdata->switchname, ch->pcdata->switchname);
  death_info(buf);
  return;
}
...
void autodrop(CHAR_DATA *ch) 
{
  ...
  else if (ch->stance[MONK_AUTODROP]==STANCE_SS4) sprintf(stancename,"pizza");
  else if (ch->stance[MONK_AUTODROP]==STANCE_SS5) sprintf(stancename,"zarius");
...
void dropinvis(CHAR_DATA *ch)
{
  ...
  if ((IS_CLASS(ch, CLASS_DROW) || IS_CLASS(ch, CLASS_DROID) || IS_CLASS(ch, CLASS_CYBORG)) && IS_SET(ch->newbits, NEW_DARKNESS))
  {
    REMOVE_BIT(ch->newbits, NEW_DARKNESS);
    REMOVE_BIT(ch->in_room->room_flags, ROOM_TOTAL_DARKNESS);
  }
