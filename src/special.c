...
/*           || victim->in_room == ch->in_room*/
	   || victim->pcdata->kingdom == 1)
	{
	  ...
	  ch->position == POS_STANDING)
        ...
        sprintf( buf,"%s is invading the Kingdom of %s!.",victim->name, kingdom_table[1].whoname); 
        do_info( ch , buf);
...
/*           || victim->in_room == ch->in_room*/
	   || victim->pcdata->kingdom == 2)
	{
	  ...
	  ch->position == POS_STANDING)
        ...
        sprintf( buf,"%s is invading the castle of %s",victim->name, kingdom_table[2].whoname); 
        do_info( ch , buf);
...
/*           || victim->in_room == ch->in_room*/
	   || victim->pcdata->kingdom == 4)
	{
	  ...
	  ch->position == POS_STANDING)
        ...
        sprintf( buf,"%s is invading the castle of %s",victim->name, kingdom_table[4].whoname); 
        do_info( ch , buf);
...
/*           || victim->in_room == ch->in_room*/
	   || victim->pcdata->kingdom == 5)
	{
	  ...
	  ch->position == POS_STANDING)
        ...
        sprintf( buf,"%s is invading the castle of %s",victim->name, kingdom_table[5].whoname); 
        do_info( ch , buf);
...
/*           || victim->in_room == ch->in_room*/
	   || victim->pcdata->kingdom == 3)
	{
	  ...
	  ch->position == POS_STANDING)
        ...
        sprintf( buf,"%s is invading the kingdom of %s",victim->name, kingdom_table[3].whoname); 
        do_info( ch , buf);
...
rnd_chat = number_range (1, 10);
   ...
   if ( rnd_chat <= 4)
                sprintf( buf, "Pain! Oh Beautiful Pain!");
   else if ( rnd_chat <= 5)
                sprintf( buf, "What is your pleasure?");
   else if ( rnd_chat <= 6)
                sprintf( buf, "We have wonderful sights to show you!");
   else if ( rnd_chat <= 7)
                sprintf( buf, "Your pain will be legendary, even in HELL!");
   else if ( rnd_chat <= 8)
                sprintf( buf, "Welcome to your fate....");
   else if ( rnd_chat <= 9)
                sprintf(buf,  "We will have your Soul!");
   else if ( rnd_chat <= 10)
                sprintf( buf, "Do you like to play?");

    do_chat( ch, buf );
        behead(victim);
        powerdown(victim);
        victim->level = 2;
        sprintf(buf,"#RPinhead #0raises his hand and #W%s #0is ripped apart by chains with hooks#n",victim->pcdata->switchname);
        do_info(ch,buf);
...
if (!obj2)
                {
                    ...
                    {
                        default:
                            sprintf(buf,"Look at what I found!");
                            do_say(ch,buf);
                            break;
                        case ITEM_FOOD:
                            sprintf(buf, "Yumm.. I'm hungry, too!");
                            do_say(ch,buf);
                        ...
                        case ITEM_STAFF:
                            ...
                            break;
                        case ITEM_WEAPON:
                            sprintf(buf,"Sweeet!  Look at this little treasure!");
                            do_say(ch,buf);
                            wear_obj(ch,object,FALSE);
                            break;
                        case ITEM_ARMOR:
...
bool spec_tax_man( CHAR_DATA *ch )
{
    ...
    {
    sn = number_range( victim->pcdata->bank * 0.08, victim->pcdata->bank * 0.1);
    victim->pcdata->bank -= sn;
    sprintf( log_buf, "Tax collector got %s for %d bones (from bank).", victim->name, sn );
    }
    else {
    sn = number_range( victim->bones * 0.08, victim->bones * 0.1);
    victim->bones -= sn;
    sprintf( log_buf, "Tax collector got %s for %d bones.", victim->name, sn );
    }
...
bool spec_fairy( CHAR_DATA *ch )
{
    ...
    */
    ...
    act( "$n says 'Hello $N, you silly little freak!!", ch, NULL, victim, TO_ROOM );

    act( "$n hugs $N, and licks $M on the cheek!", ch, NULL, victim, TO_NOTVICT );
    act( "$N hugs you, and licks you on the cheek!", victim, NULL, ch, TO_CHAR );
    
    act( "$n waves her bloody chicken bone towards $N.", ch, NULL, victim, TO_NOTVICT );
    act( "$N waves her bloody chicken bone towards you.", victim, NULL, ch, TO_CHAR );
    SET_TIMER(victim, TIMER_EXPERIENCE, 10);

    send_to_char( "You feel at ease with your experience and learn quicker.\n\r", victim );
    act( "$n starts learning from their mistakes quicker.", victim, NULL, NULL, TO_ROOM );
    return TRUE;
}
