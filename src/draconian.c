...
void do_dragonorb( CHAR_DATA *ch, char *argument )
{
	...
	stc("#G  \\       #Y\\\\#g_)  #R`#g-\\#Y   |      /\n\r",ch);
	stc("#G   )       #g/'  /^^^  #P___   #Y/\n\r",ch);
	sprintf(buf,"#G  /__     #g/   \\'    #P/  #W*#P\\            #yY#Yellow: #R+#Y(#G%d#Y)#R+\n\r",ch->pcdata->powers[ORB_YELLOW]);
	stc(buf,ch);
	sprintf(buf,"#G /   `\\  #g|  \\  #G|#y-- #p|     #P| #Y---        #gG#Green: #R+#Y(#G%d#Y)#R+\n\r",ch->pcdata->powers[ORB_GREEN]);
	stc(buf,ch);
	stc("#G       |/#g\\   \\  \\_#G__#p\\,_,#P/\n\r",ch);
	stc("#G       |   #g\\   \\----#G---/'  #Y\\\n\r",ch);
	stc("#g             \\   \\    #Y|      \\\n\r",ch);
	...
	{
	    sprintf(buf,"#GYou've tapped as much power from the #y%s #GDragon Orb as you can!#n\n\r", arg1);
	    send_to_char(buf,ch);
	    return;
	}
	...
	if ( cost > ch->exp )
	{
	    sprintf(buf,"#GYou require #y%d #Gexp to tap the #y%s #GDragon Orb!#n\n\r", cost, arg1);
	    send_to_char(buf,ch);
	    return;
	}
	...
	ch->exp -= cost;
	sprintf(buf,"#GYou successfully tap the energies from the #y%s #GDragon Orb!#n\n\r", arg1);
	send_to_char(buf,ch);
    }
...
void do_spikes( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp(arg1,"release"))
	{
		...
		sprintf(buf,"#GYour spikes shoot from your body, #Rimpaling #y$N!  #0[#R%d#0]#n",dam);
		act(buf,ch,NULL,victim,TO_CHAR);
		sprintf(buf,"#GA plethora of sharp spikes shoot from #y$n's #Gbody, #Rimpaling #Gyou!  #0[#R%d#0]#n",dam);
	...
	else if (!str_cmp(arg1,"grow"))
	{
		...
		if (is_number(arg2))
		{
			...
			if (urin_counter>1)
			{
				sprintf( buf, "#GYou gain #y%d #Gspikes.#n\n\r", urin_counter );
				send_to_char( buf, ch );
			}
		return;
		}		
...
if (stun < 60)
{
	...
	sprintf(buf,"#GYou bash your head into #y$N's! #R*#yCraCk#R* #0[#R%d#0]#n",dam);
	act(buf,ch,NULL,victim,TO_CHAR);
	sprintf(buf,"#y$n #Gbashes $s head into yours! #R*#yCraCk#R* #0[#R%d#0]#n",dam);
	act(buf,ch,NULL,victim,TO_VICT);
...
void do_lizardform( CHAR_DATA *ch, char *argument ) 
{
	...
	if (ch->generation == 6)
	sprintf(buf, "#R%s#G, the scrawny lizard man#n",ch->pcdata->switchname);
	else if (ch->generation == 5)
	sprintf(buf, "#R%s#G, the muscular lizard man#n",ch->pcdata->switchname);
	else if (ch->generation == 4)
	sprintf(buf, "#R%s#G, the giant lizard#n",ch->pcdata->switchname);
	else if (ch->generation == 3)
	sprintf(buf, "#R%s#G, the vicious draconian#n",ch->pcdata->switchname);
	else
	sprintf(buf, "#R%s#G, the bloodthirsty draconian#n",ch->pcdata->switchname);
    free_string( ch->morph );
...
void do_venomspit(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf,"#0You spit a #Gvenomous substance #0at #R$N!  #0[#R%d#0]#n",dam);
	act(buf,ch,NULL,victim,TO_CHAR);
	sprintf(buf,"#R$n #0spits a #Gvenomous substance #0at you!  #0[#R%d#0]#n",dam);
	act(buf,ch,NULL,victim,TO_VICT);
...
void do_gasbreath(CHAR_DATA *ch, char *argument)
{
	...
	if ( IS_NPC(ch) ? !IS_NPC(vch) : IS_NPC(vch) )
	{
	   ...
	   if ( IS_AFFECTED(vch, AFF_SANCTUARY) ) dam /=2;
		...
		sprintf(buf,"#0Your gasbreath overcomes #R%s!  #0[#R%d#0]#n\n\r",vch->short_descr,dam);
		stc(buf,ch);
...
void do_firebreath(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf,"#gYou rear back your head and blow forth a huge ball of #Rf#Yl#Wa#Ym#Re #gat #G$N!  #0[#R%d#0]#n",dam);
	act(buf,ch,NULL,victim,TO_CHAR);
	sprintf(buf,"#G$n #grears back $s head and blows forth a huge ball of #Rf#Yl#Wa#Ym#Re #gat you!  #0[#R%d#0]#n",dam);
	act(buf,ch,NULL,victim,TO_VICT);
...
void draconian_death( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if (IS_SET(victim->pcdata->stats[1],DRACONIAN_ACIDBATH))
	{
		...
		sprintf(buf,"#G$N's #0body #REXPLODES, #0splashing #Ghot acid #0all over you!  #0[#R%d#0]#n",dam2);
		act(buf,ch,NULL,victim,TO_CHAR);
		sprintf(buf,"#0Your body #REXPLODES, #0splashing #Ghot acid #0all over #G%s!  #0[#R%d#0]#n\n\r",ch->pcdata->switchname,dam2);
		stc(buf,victim);
	...
	else if (IS_SET(victim->pcdata->stats[1],DRACONIAN_FLAMEDEATH)) //Flaming Death : Good dmg, fire affect.
	{
		...
		sprintf(buf,"#R$N's #0body #rEX#RP#7LO#RD#rES #0in #Rf#Yl#Wam#Ye#Rs, #0searing your flesh!  [#R%d#0]#n",dam);
		act(buf,ch,NULL,victim,TO_CHAR);
		sprintf(buf,"#0Your body #rEX#RP#7LO#RD#rES #0in #Rf#Yl#Wam#Ye#Rs, #0searing #R$n's #0flesh!  [#R%d#0]#n",dam);
		stc(buf,victim);
		...
		sprintf(buf,"#r%s #Rcatches #Yfire!#n\n\r",ch->pcdata->switchname);
		stc(buf,victim);
		}
	}
	...
	else if (IS_SET(victim->pcdata->stats[1],DRACONIAN_PETRIFY)) //Petrify: ch loses wielded wpn, and is 'stunned'
	{
		...
		sprintf(buf,"#0Your body turns to #7stone, #0petrifying #W%s!#n\n\r",ch->pcdata->switchname);
		stc(buf,victim);
		...
		if ( (obj= get_eq_char(ch,WEAR_WIELD) ) != NULL)
		{
			...
			sprintf(buf,"#7%s's #0weapon is wrenched from their hands as your corpse falls limp to the ground!#n\n\r",ch->pcdata->switchname);
			stc(buf,victim);
	...
	else if (IS_SET(victim->pcdata->stats[1],DRACONIAN_CHLOROFORM)) //Chloroform Effusion: gasses room and chance to stun/poison
	{
		...
		if ( !IS_NPC(vch) && vch != victim )
		{
			...
			sprintf(buf,"#0Your #Pchloroform gas #0overcomes #R%s!  #0[#R%d#0]#n\n\r",vch->pcdata->switchname,dam);
			stc(buf,victim);
			sprintf(buf,"#R%s's #Pchloroform gas #0overcomes you!  #0[#R%d#0]#n\n\r",victim->pcdata->switchname,dam);
			stc(buf,vch);
