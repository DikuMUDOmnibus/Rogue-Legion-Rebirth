...
#include "merc.h"
     ...
     void add_affect ( OBJ_DATA * obj, int apply, long value )
	{
     ...
     void rare_eq1 ( CHAR_DATA * ch, CHAR_DATA * victim )
        ...
        SET_BIT( obj->spectype2, XITEM_RARE );
	SET_BIT( obj->spectype2, XITEM_UNID );
     ...
     void common_eq1 ( CHAR_DATA * ch, CHAR_DATA * victim )
        ...
        SET_BIT( obj->quest, QUEST_RELIC );
        SET_BIT( obj->spectype2, XITEM_COMMON );
        SET_BIT( obj->wear_flags, ITEM_TAKE );
        obj->questowner = str_dup( ch->name );
        wear = number_range( 1, 13 );
        switch ( wear )
     ...
     void rare_weapon1 ( CHAR_DATA * ch, CHAR_DATA * victim )
        ...
        add_affect( obj, APPLY_MOVE, move );
        add_affect( obj, APPLY_AC, armor );
        obj->value[1] = number_range (80,90);
        obj->value[2] = number_range (100,100);

        SET_BIT( obj->quest, QUEST_RELIC );
        SET_BIT( obj->spectype2, XITEM_RAREWEAP );
	SET_BIT( obj->spectype2, XITEM_UNID );
        SET_BIT( obj->wear_flags, ITEM_TAKE );
        obj->questowner = str_dup( ch->name );
...
/*        if ( IS_SET( obj->spectype2, XITEM_UNID ) )
        {
        send_to_char("That item has not been identified yet.\n\r", ch);
        return;
        }
*/
	if ( IS_SET( obj->spectype2, XITEM_COMMON ) ) 
	{	
	...
	return;
        ...
        if ( IS_SET( obj->spectype2, XITEM_RARE ) )
        {
	...
	send_to_char(buf, ch);
        REMOVE_BIT(obj->spectype2, XITEM_UNID);
     	return;
        }
     ...
     void do_weapsell(CHAR_DATA *ch, char *argument)
        ...
        if ( !IS_SET( obj->spectype2, XITEM_RAREWEAP ) )
        {
