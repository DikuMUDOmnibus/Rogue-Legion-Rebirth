...
void do_bstab( CHAR_DATA *ch, char *argument)
	...
	return;
		...
		raw_kill(victim);
		sprintf(buf,"#r%s #0wipes the #Rblood #0from his #7dagger #0after making another skillfull #Rassa#rssina#Rtion#n.",ch->pcdata->switchname);
		do_info(ch,buf);
		return;
		}
	    ...
	    sprintf( buf, "#0You step out of the shadows behind #R$N #0and drive a #7dagger #0into $S back!#n   #r[#R%d#r]#n\n\r#n", dam);
		act(buf, ch, NULL, victim, TO_CHAR);
		sprintf( buf, "#0You scream in #Ragony #0as you feel a sharp #7dagger #0plunge into your back!   #r[#R%d#r]#n\n\r", dam);
		stc(buf, victim);
		ch->move -= 10000;
...
void do_tdagger( CHAR_DATA *ch, char *argument)
	...
	return;
		...
		if (chance >= 95) //Head Shot
		{
			...
			sprintf( buf, "#0You hurl a #7dagger #0at #R$N, #0hitting $M right in #S forehead!#n   #r[#R%d#r]#n", dam);
			act(buf, ch, NULL, victim, TO_CHAR);
			sprintf( buf, "#0A #7dagger #0flies out of nowhere and hits you in the forehead!  #RSHIT that fucking hurt!#n  #r[#R%d#r]#n", dam);
			stc(buf, victim);
		}
		...
		else if (chance >= 90) //Neck Hit
		{
			...
			sprintf( buf, "#0You hurl a #7dagger #0at #R$N, penetrating #0$S neck!#n   #r[#R%d#r]#n", dam);
			act(buf, ch, NULL, victim, TO_CHAR);
			sprintf( buf, "#0A #7dagger #0flies out of nowhere and #Rimpales #0your neck!  #RSHIT that fucking hurt!#n  #r[#R%d#r]#n\n\r", dam);
			stc(buf, victim);
		...
		else if (chance >= 30) //General Chest Hit
		{
			...
			sprintf( buf, "#0You hurl a #7dagger #0at #R$N, piercing #0into $S chest!#n   #r[#R%d#r]#n", dam);
			act(buf, ch, NULL, victim, TO_CHAR);
			sprintf( buf, "#0A #7dagger #0flies out of nowhere and #Rpierces #0deep into your chest!#n  #r[#R%d#r]#n\n\r#n", dam);
			stc(buf, victim);
		}
		...
		else if (chance >= 2) //Leg Hit
		{
			...
			sprintf( buf, "#0You hurl a #7dagger #0at #R$N, digging #0into $S leg!#n   #r[#R%d#r]#n", dam);
			act(buf, ch, NULL, victim, TO_CHAR);
			sprintf( buf, "#0A #7dagger #0flies out of nowhere and #Rdigs #0deep into your leg!#n  #r[#R%d#r]#n\n\r#n", dam);
			stc(buf, victim);
		}
...
sprintf( buf, "#0You hurl a #7dagger #0at #R$N, #0hitting him in a rather.. uncompromising location!#n   #r[#R%d#r]#n", dam);
act(buf, ch, NULL, victim, TO_CHAR);
sprintf( buf, "#RYou whimper pitifully as you take a throwing dagger to the crotch!#n  #r[#R%d#r]#n\n\r#n", dam);
stc(buf, victim);
sprintf(buf,"#0HAHA #R%s #0has just taken a throwing dagger to the crotch!#n",IS_NPC(victim) ? victim->short_descr : victim->pcdata->switchname);
do_info(victim,buf);
		}
   ...
   if (randnum == 1) //<----Disarm
{
	if(((obj = get_eq_char( victim, WEAR_WIELD ) ) == NULL) || obj->item_type != ITEM_WEAPON)
  {
	...
	if (successchance >= 50)
	{
	sprintf(buf,"#R$n #0slices $s #7dagger #0sneakily accross your wrist, causing you too lose your grip!#n\n\r");
	act( buf, ch, NULL, victim, TO_VICT    );
	sprintf(buf,"#0You sneakily slice your dagger across #R$N's #0wrist, causing them to lose their grip!#n\n\r");
	act( buf,  ch, NULL, victim, TO_CHAR    );
	...
	{
    sprintf(buf,"#R$n #0kicks your shins, causing your legs to buckle!#n\n\r");
    act( buf, ch, NULL, victim, TO_VICT    );
    sprintf(buf,"#0You kick #R$N #0in the shins, causing $S legs to buckle!#n\n\r");
    act( buf,  ch, NULL, victim, TO_CHAR    );
    sprintf(buf,"#R$n #0kicks #R$N #0in the shins, causing $S legs to buckle!#n\n\r");
  ...
  if (successchance >= 50)
  {
    sprintf(buf,"#R$n BASHES #0his jeweled dagger hilt into your face! You reel back, stunned!#n\n\r");
    act( buf, ch, NULL, victim, TO_VICT    );
    sprintf(buf,"#0You BASH your dagger hilt into #R$N's #0face!#n\n\r");
    act( buf,  ch, NULL, victim, TO_CHAR    );
    sprintf(buf,"#R$n #RBASHES #0his dagger hilt into #R$N's #0face, causing them to reel back stunned!#n\n\r");
    act( buf,  ch, NULL, victim, TO_NOTVICT );
