...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    if (IS_SET(obj->spectype2, XITEM_RARE)) strcat( buf, "#0(#GR#gar#Ge#0)#n " );
    if (IS_SET(obj->spectype2, XITEM_COMMON)) strcat( buf, "#7(#LB#la#7s#li#Lc#7)#n ");
    if (IS_SET(obj->spectype2, XITEM_BREATH)) strcat( buf, "#R(#0Blazing#R)#n ");
    if (obj->item_type == ITEM_WEAPON)
        {
                sprintf(lvl, "#C(Level #R%d#C)#n", obj->weapon_level);
                strcat(buf, lvl);
        }
  ...
  if (IS_CLASS(ch, CLASS_VAMPIRE))
	{
	  if(ch->played > 0)
      {
        ...
        if(IS_CLASS(ch, CLASS_SHINOBI)){
                if(ch->chakra > 3) value += ch->chakra / 50;
        }
...
int char_ac(CHAR_DATA *ch)
{
   ...
   if( IS_AWAKE(ch) )
	...
	value -= (ch->pcdata->stats[DRACONIAN_SPIKES] * 2);
        ...
        if(IS_CLASS(ch,CLASS_SHINOBI)){
                if(ch->chakra > 2) value -= ch->chakra / 2.5;
        }
    if (IS_CLASS(ch, CLASS_SKYBLADE))
  {
...
void show_list_to_char( OBJ_DATA *list, CHAR_DATA *ch, bool fShort, bool fShowNothing )
{
	...
	if ( IS_NPC(ch) || IS_SET(ch->act, PLR_COMBINE) )
	{
	    if ( prgnShow[iShow] != 1 )
	    {
		sprintf( buf, "#R(#W%2d#R)#n ", prgnShow[iShow] );
		send_to_char( buf, ch );
	    }
	    else
	    {
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if (!IS_NPC( victim ))
    {
	if (ch->symbiotetype == 1) act("$N has a #GL#gif#Ge#n Symbiote.", ch, NULL, victim, TO_CHAR);
        if (ch->symbiotetype == 2) act("$N has a #7D#0e#7a#0t#7h#n Symbiote.", ch, NULL, victim, TO_CHAR);
...
!(IS_CLASS(ch, CLASS_DROW)))
	...
	{	
	    ...
	    {
		sprintf(buf, "#w     You see a shimmering wall of many #sc#so#sl#so#su#sr#ss #w%s.\n\r#n", exitname2[door]);
	        send_to_char(buf, ch);
	    }
    ...
    case ITEM_SLOT_MACHINE:
           if(obj->value[3] == 1)
             ...
             partial = FALSE;
           sprintf(buf,"This is a %d bar slot machine costing %d bones with a jackpot of %d bones.\n\rThe jackpot has %spartial winnings.\n\r", obj->value[2], obj->value[0], obj->value[1],  partial ? "" : "no ");
           send_to_char(buf, ch);
           break;
...
void do_score( CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_PRIEST))
  {
	...
	sprintf( buf, "#R[#nYou have #C%d#n drow power points stored#R]\n\r",
    ...
    if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_MONK) )
    {
      ...
      stc(buf,ch);
      sprintf(buf,"#R[#nYour current focus points:       #C%d#R]\n\r",ch->focus[CURRENT]);
      stc(buf,ch);
      sprintf(buf,"#R[#nYour maximum focus points:       #C%d#R]\n\r",ch->focus[MAXIMUM]);
      stc(buf,ch);
    }
    ...
    if( !IS_NPC(ch) )
    {
	sprintf( buf, "#R[#nYou have done #C%d #nquests for #C%d #nbones#R]\n\r", ch->pcdata->questsrun, ch->pcdata->questtotal );
	stc( buf, ch);
    }
...
void do_who(CHAR_DATA *ch, char *argument)
{
  ...
  for (d = descriptor_list; d != NULL; d = d->next)
  {
    ...
    if ( wch->sex == 1 )
                    {
                        sprintf ( sexbuf, "#B[#n%s#B]#n ", "#cM" );
                    }
                    if ( wch->sex == 2 )
                    {
                        sprintf ( sexbuf, "#B[#n%s#B]#n ", "#pF" );
                    }
                    if ( wch->sex == 0 )
                    {
                        sprintf ( sexbuf, "#B[#n%s#B]#n ", "#wN" );
                    }
	...
	case MAX_LEVEL -  0:
			...
			else if (!str_cmp(wch->pcdata->switchname,"Magus"))
			class = "   #r-#R=#wCoder#R=#r-#n    ";
            else 
            class = "     IMP     "; 
            break;
	case MAX_LEVEL -  1: 
  ...
  if (IS_CLASS(wch, CLASS_MAGE) )
    ...
    if (wch->generation == 1)
      ...
      {
    if (wch->generation == 1)
          sprintf(kav,"%s#0Killing Machine%s    ",openb,closeb);
    else if(wch->generation == 2)
          sprintf(kav,"%s#0Terminator%s         ",openb,closeb);
        else
          sprintf(kav,"%s#0Bionic Man%s         ",openb,closeb);
      }
    else if( IS_CLASS(wch, CLASS_PRIEST) )
  {
    if (wch->generation == 1)
          sprintf( kav, "%s#WHoly Priest#n%s        ", openb, closeb );
    else
          sprintf( kav, "%s#WPriest#n%s             ", openb, closeb );
  }
...
sprintf(kav,"%s\\#0|%s/#l-%sK#0ag%se#l-%s\\#0|%s/#n            ", buf19, buf19,buf19,buf19,buf19,buf19);
    ...
    if (wch->generation == 1)
      ...
      {
    if (wch->generation == 1)
          sprintf( kav, "%s#CJedi #cMaster#n%s        ", openb, closeb );
    else if (wch->generation == 2)
          sprintf( kav, "%s#CJedi #BKnight#n%s        ", openb, closeb );
    else if (wch->generation == 3)
          sprintf( kav, "%s#BForce #RLord#n%s         ", openb, closeb );
    else if (wch->generation == 4)
          sprintf( kav, "%s#CJedi#n%s               ", openb, closeb );
    else if (wch->generation == 5)
          sprintf( kav, "%s#BForce #RMaster#n%s       ", openb, closeb );
    else if (wch->generation == 6)
          sprintf( kav, "%s#0Jedi #BTrainee#n%s       ", openb, closeb );
    else
          sprintf( kav, "%s#CSpoon Bender#n%s       ", openb, closeb );
      }
  else if( IS_CLASS(wch, CLASS_ANGEL) )
      {
    if (wch->generation == 1)
      ...
      {
    if (wch->generation == 1)
      sprintf( kav, "%s#7Murderer#n%s           ", openb, closeb );
    else if (wch->generation == 2)
      sprintf( kav, "%s#7Slicer#n%s             ", openb, closeb );
    else if (wch->generation == 3)
      sprintf( kav, "%s#7Violator#n%s           ", openb, closeb );
    else if (wch->generation == 4)
      sprintf( kav, "%s#0Assassin#n%s           ", openb, closeb );
    else if (wch->generation == 5)
      sprintf( kav, "%s#0Slinker#n%s            ", openb, closeb );
        else 
      sprintf( kav, "%s#0Nightstalker#n%s       ", openb, closeb );
}
  else if( IS_CLASS(wch, CLASS_SKYBLADE) )
  {
    if (wch->generation == 1)
      sprintf( kav, "%s#WWind Sword#n%s         ", openb, closeb );
    else if (wch->generation == 2)
      sprintf( kav, "%s#WCloud Defender#n%s     ", openb, closeb );
    else if (wch->generation == 3)
      sprintf( kav, "%s#WCloud #0Knight#n%s       ", openb, closeb );
    else if (wch->generation == 4)
      sprintf( kav, "%s#WTempest Knight#n%s     ", openb, closeb );
    else if (wch->generation == 5)
      sprintf( kav, "%s#WSkyBlade#n%s           ", openb, closeb );
    else if (wch->generation == 6)
       sprintf( kav, "%s#CSky#wTrainee#n%s         ", openb, closeb );
        else 
      sprintf( kav, "%s#CSky#WWimp#n%s             ", openb, closeb );
  }
  else if( IS_CLASS(wch, CLASS_THIEF) )
  {
    if (wch->generation == 1)
      sprintf( kav, "%s#wM#0a#ws#0t#we#0r #wT#0h#wi#0e#wf#n%s         ", openb, closeb );
    else if (wch->generation == 2)
      sprintf( kav, "%s#wSilent #0Killer#n%s        ", openb, closeb );
    else if (wch->generation == 3)
      sprintf( kav, "%s#0Thief#n%s                ", openb, closeb );
    else if (wch->generation == 4)
      sprintf( kav, "%s#0Burglar#n%s              ", openb, closeb );
    else if (wch->generation == 5)
      sprintf( kav, "%s#wNight #0Blade#n%s          ", openb, closeb );
    else if (wch->generation == 6)
       sprintf( kav, "%s#0Cut#wPurse#n%s            ", openb, closeb );
        else 
      sprintf( kav, "%s#C0Pick Pocket#n%s         ", openb, closeb );
  }
  else if( IS_CLASS(wch, CLASS_GHOUL) )
      {
    if (wch->generation == 1)
      sprintf( kav, "%s#gNec#Grite#n%s            ", openb, closeb );
    else if (wch->generation == 2)
      sprintf( kav, "%s#wSwamp #0Thing#n%s        ", openb, closeb );
    else if (wch->generation == 3)
      sprintf( kav, "%s#gUndead#n%s             ", openb, closeb );
    else if (wch->generation == 4)
      sprintf( kav, "%s#0Zom#wbie#n%s             ", openb, closeb );
    else if (wch->generation == 5)
      sprintf( kav, "%s#GFlesh #0Eater#n%s        ", openb, closeb );
    else if (wch->generation == 6)
      sprintf( kav, "%s#gGhoul#n%s              ", openb, closeb );
        else 
      sprintf( kav, "%s#GScum Bag#n%s           ", openb, closeb );
}
  else if( IS_CLASS(wch, CLASS_DRACONIAN) )
      {
    if (wch->generation == 1)
      sprintf( kav, "%s#gDragon #Ylord#n%s          ", openb, closeb );
    else if (wch->generation == 2)
      sprintf( kav, "%s#wWar #BCaptain#n%s          ", openb, closeb );
    else if (wch->generation == 3)
      sprintf( kav, "%s#gWar #0General#n%s          ", openb, closeb );
    else if (wch->generation == 4)
      sprintf( kav, "%s#0Slave #wDriver#n%s         ", openb, closeb );
    else if (wch->generation == 5)
      sprintf( kav, "%s#GBig Lizard#n%s           ", openb, closeb );
    else if (wch->generation == 6)
      sprintf( kav, "%s#gWhelp#n%s                ", openb, closeb );
        else 
      sprintf( kav, "%s#GLizard#n%s               ", openb, closeb );
      }
  else if( IS_CLASS(wch, CLASS_UNDEAD_KNIGHT) )
      {
        if (wch->generation == 1)
          sprintf( kav, "%s#BFallen Paladin#n%s       ", openb, closeb );
        else if (wch->generation == 2)
          sprintf( kav, "%s#BUndead Lord#n%s          ", openb, closeb );
        else if (wch->generation == 3)
          sprintf( kav, "%s#BUndead Knight#n%s        ", openb, closeb );
        else if (wch->generation == 4)
          sprintf( kav, "%s#BUndead Knight#n%s        ", openb, closeb );
        else if (wch->generation == 5)
      ...
      sprintf(     kav, "%s#LUndead Knight#n%s        ", openb, closeb );
          ...
          sprintf( kav, "%s#BSkeleton Knight#n%s      ", openb, closeb );
      }
      else if ( IS_CLASS(wch, CLASS_WEREWOLF))
      {
        if (wch->generation == 1)
          sprintf( kav, "%s#BChieftain#n%s            ", openb, closeb );
        else if (wch->generation == 2)
          sprintf( kav, "%s#BTribe Shaman#n%s         ", openb, closeb );
        else if (wch->generation == 3)
          sprintf( kav, "%s#BPack Leader#n%s          ", openb, closeb );
        else if (wch->generation == 4)
          sprintf( kav, "%s#BFenris Wolf#n%s          ", openb, closeb );
        else if (wch->generation == 5)
          sprintf( kav, "%s#BStalker#n%s              ", openb, closeb );
        else if (wch->generation == 6)
          sprintf( kav, "%s#BHunter#n%s               ", openb, closeb );
        else if (wch->generation == 7) 
          sprintf( kav, "%s#BPup of Gaia#n%s          ", openb, closeb );
        else
          sprintf( kav, "%s#BHalfbreed Bastard#n%s    ", openb, closeb );
      }
      else if ( IS_CLASS(wch, CLASS_MONK))
      {
        if (wch->generation == 1)
          sprintf(kav,"%s#bAbbot#n%s              ", openb, closeb);   
        else if (wch->generation == 2)
          sprintf(kav,"%s#bHigh Priest#n%s        ", openb, closeb);   
        else if (wch->generation == 3)
          sprintf(kav,"%s#bPriest#n%s             ", openb, closeb);   
        else if (wch->generation == 4)
          sprintf(kav,"%s#bFather#n%s             ", openb, closeb);   
        else if (wch->generation == 5)
          sprintf(kav,"%s#bMonk#n%s               ", openb, closeb);   
        else if (wch->generation == 6)
          sprintf(kav,"%s#bBrother#n%s            ", openb, closeb);   
        else if (wch->generation == 7)
          sprintf(kav,"%s#bAcolyte#n%s            ", openb, closeb);   
        else
          sprintf(kav,"%s#bFanatic#n%s            ", openb, closeb);
      }
      else if ( IS_CLASS(wch, CLASS_DROW)) 
      {
        if (wch->generation == 1)
      ...
      else if (IS_CLASS(wch, CLASS_LICH))
      {
        if (wch->generation == 1)
          ...
          sprintf( kav, "%s#7Lich Bitch#n%s         ", openb, closeb );
        else if (wch->generation == 3)
      ...
      {
        if (wch->generation == 1)
          sprintf( kav, "%s#RShire Mayor#n%s        ", openb, closeb );
        else if (wch->generation == 2)
          sprintf( kav, "%s#RCouncil Commitee#n%s   ", openb, closeb );
        else if (wch->generation == 3)
          sprintf( kav, "%s#RMerchant#n%s           ", openb, closeb );
        else if (wch->generation == 4)
          sprintf( kav, "%s#RCitizen#n%s            ", openb, closeb );
        else if (wch->generation == 5)
          sprintf( kav, "%s#RFarmer#n%s             ", openb, closeb );
        else if (wch->generation == 6)
          sprintf( kav, "%s#RHobbit#n%s             ", openb, closeb );
        else
          sprintf( kav, "%s#RBumpkin#n%s            ", openb, closeb );
      }
      else if (IS_CLASS(wch, CLASS_DRONE))
      {
        if (wch->generation == 1)
      ...
      else if ( IS_CLASS(wch, CLASS_SAMURAI) )
      {
        if (wch->generation == 1)
          ...
          sprintf(kav, "%s#YSamu#Yrai Mas#Rter#n%s       ", openb, closeb);
        else if (wch->generation == 4)
    ...
    if (wch->level > 6 )
    {
	sprintf( buf3 + strlen(buf3), "#0[#n%s#0]%s %s %s %s %s %s\n\r",
	    class, invisbuf,
	    kav,
	...
	else if ( wch->level >= 3 && wch->level <= 6 )
    {
          if ( wch->race >= 43 )
          {
            sprintf( buf4 + strlen(buf4), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
              pkratio,
              kav,
              wch->pcdata->switchname,
      ...
      {
            sprintf( buf6 + strlen(buf6), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
      ...
      {
            sprintf( buf7 + strlen(buf7), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
      ...
      {
            sprintf( buf8 + strlen(buf8), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
      ...
      {
            sprintf( buf9 + strlen(buf9), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
      ...
      {
            sprintf( buf10 + strlen(buf10), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
      ...
      {
            sprintf( buf11 + strlen(buf11), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
...
faith);
      ...
      {
            sprintf( buf12 + strlen(buf12), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio, 
              kav,
              wch->pcdata->switchname,
 ...
 faith);
      ...
      {
            sprintf( buf13 + strlen(buf13), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
...
faith);
      ...
      {
            sprintf( buf14 + strlen(buf14), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio, 
              kav,
              wch->pcdata->switchname,
...
faith);
      ...
      {
            sprintf( buf15 + strlen(buf15), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
...
faith);
      ...
      {
            sprintf( buf16 + strlen(buf16), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
...
faith);
      ...
      {
            sprintf( buf17 + strlen(buf17), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
...
faith);
      ...
      {
            sprintf( buf18 + strlen(buf18), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
...
faith);
      ...
      {
            sprintf( buf19 + strlen(buf19), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
 ...
 faith);
      ...
      {
            sprintf( buf20 + strlen(buf20), "%-12s%s%s %-28s%-12s %s %s\n\r",
              class, sexbuf,
		pkratio,
              kav,
              wch->pcdata->switchname,
...
faith);
    ...
    {
        sprintf( buf5 + strlen(buf5), "%-12s%s%s %-28s%-12s %s %s\n\r",
            class, sexbuf,
		pkratio,
            kav,
            wch->pcdata->switchname,
 ...
 faith);
  ...
  send_to_char(buf, ch);
    send_to_char("                     #CThe Rogue Legion #Rv1.0#n\n\r", ch);
      mains = TRUE;
  }
    ...
    if ( imm )
  {
      if ( !immortal )
      { 
      sprintf(buf, "\n\r %s\n\r", get_dystopia_banner("#0Gods of #CThe Rogue Legion #R1.0#n", 80));
    send_to_char(buf, ch);
      immortal = TRUE; 
      }
      stc(buf3,ch);
...
//  return;
  ...
  b, c, (mudinfo[MUDINFO_MCCP_USERS] + mudinfo[MUDINFO_OTHER_USERS]) / mudinfo[MUDINFO_UPDATED],  mudinfo[MUDINFO_PEAK_USERS]);
  send_to_char(buf, ch);
...
// New stuff starts here -Xrak
  ...
  c = c * 10;
  c = b - c;
  sprintf(buf, "    #n[#YP#ylayers #YS#yince #YM#yarch#7 1, 2008#n:#7 %d#n][#YD#yatatransfer #n:#Y A#yverage #YR#yate#c: #n%d.%d #Yk#yb/#Ys#yec#n]\n\r",
  top_playerid, b / 10, c);
  send_to_char(buf, ch);
...
//       WAIT_STATE(ch,36);
	...
	if(ch->tells)
      {
         sprintf( buf, "AFK mode removed.  You have #R%d#n tells waiting.\n\r", ch->tells );
         send_to_char( buf, ch );
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->pcdata->learned[sn] >= SKILL_ADEPT )
	{
		...
		ch->exp < 10000 )
	{
	    sprintf( buf, "You need 10000 exp to increase %s any more.\n\r",
		(skill_table[sn].name));
	    send_to_char( buf, ch );
	}
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( !IS_SET(ch->deaf, CHANNEL_TELL)
        ...
        if(!IS_NPC(ch))
        {
        	...
        	{
        sprintf( buf, "#0Your pager is set to #W%d#0 lines\n\r", ch->pcdata->pagelen + 2);
        send_to_char( buf, ch);
                }
                else
                {
...
void do_finger( CHAR_DATA *ch, char *argument )
{
  ...
  {
    sprintf(buf2, "%s\n\r", get_dystopia_banner("mcloud 2 Mud", 68));
...
void do_viewtier( CHAR_DATA *ch, char *argument )
{
	...
	if (d2->character->invis_level > get_trust(ch)) continue;
        ...
        sprintf(buf10, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf10,ch);
		}
		}
   ...
   if (d2->character->tier == 9)
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf9, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf9,ch);
		}
		}
 ...
 if (d2->character->tier == 8)
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf8, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf8,ch);
		}
		}
   ...
   if (d2->character->tier == 7)
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf7, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf7,ch);
		}
		}
 ...
 if (d2->character->tier == 6)
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf6, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf6,ch);
		}
		}
	...
	if (d2->character->level > 6) continue;
		...
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf5, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf5,ch);
		}
		}
	...
	if (d2->character->level > 6) continue;
		...
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf4, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf4,ch);
		}
		}
	...
	if (d2->character->level > 6) continue;
		...
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf3, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf3,ch);
		}
		}
	...
	if (d2->character->level > 6) continue;
		...
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf2, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf2,ch);
		}
		}
	...
	if (d2->character->level > 6) continue;
		...
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf1, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf1,ch);
		}
		}
	...
	if (d2->character->level > 6) continue;
		...
		{
		wch = ( d2->original != NULL) ? d2->original : d2->character;
        sprintf(buf0, "#c%14s                     #RS#rtatus: #c%4d\n\r", wch->pcdata->switchname, wch->race); 
        stc(buf0,ch);
		}
		}
	return;
