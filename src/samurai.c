...
void lightning_sword (CHAR_DATA *ch)
{
	...
	sprintf(buf,"Your lightning blast slams into $N's and crackles around $S body sending them sprawling. #R[#C#b %d #n#R]#n", dmg);
		act(buf, ch, NULL, victim, TO_CHAR);
	sprintf(buf,"$n's lightning blast slams into you and crackles around your body sending you sprawling. #R[#C#b %d #n#R]#n", dmg);
...
void fire_sword (CHAR_DATA *ch)
{
	...
	sprintf(buf,"Your cone of flame engulfs $N's in a sheet of flame. #R[#R#b %d #n#R]#n", dmg);
	act(buf, ch, NULL, victim, TO_CHAR);
	sprintf(buf,"$n's cone of flame engulfs you in a sheet of flame. #R[#R#b %d #n#R]#n", dmg);
	act(buf, ch, NULL, victim, TO_VICT);
...
void ice_sword (CHAR_DATA *ch)
{
	...
	if (!froze)		
	{
		sprintf(spc,"but $N recovers.");
		sprintf(spcV,"but you recover.");
	}
	else
	{
		sprintf(spc,"and $N freezes solid.");
		sprintf(spcV,"and you freeze solid.");
	}
		
	dmg = number_range(ch->wpn[1], ch->wpn[1] * 4);
	
	sprintf(buf,"Your ray of frost envelopes $N's in a block of ice, %s #R[#L#b %d #n#R]#n", spc, dmg);
	act(buf, ch, NULL, victim, TO_CHAR);
	sprintf(buf,"$n's ray of frost envelopes you in a block of ice, %s #R[#L#b %d #n#R]#n", spcV,dmg);
	act(buf, ch, NULL, victim, TO_VICT);
	sprintf(buf,"$n's ray of frost envelopes $N's in a block of ice, %s", spc);
...
void drain_sword (CHAR_DATA *ch)
{
	...
	sprintf(buf,"Your wave of energy enters $N's body and drains %d health painfully from $N. #R[#R#b %d #n#R]#n", heal, dmg);
	act(buf, ch, NULL, victim, TO_CHAR);
	sprintf(buf,"$n's wave of energy enters your body and drains you painfully. #R[#R#b %d #n#R]#n", dmg);
	act(buf, ch, NULL, victim, TO_VICT);
...
void soul_sword (CHAR_DATA *ch)
{
	...
	sprintf(buf,"$N screams as $E is ripped apart by the dark energy. #R[#y#b MORTAL #n#R]#n");
	act(buf, ch, NULL, victim, TO_CHAR);
	sprintf(buf,"$n looks on as you start screaming while dark energy rips you apart from the inside out. #R[#y#b MORTAL #n#R]#n");
	act(buf, ch, NULL, victim, TO_VICT);
