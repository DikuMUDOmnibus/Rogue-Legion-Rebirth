...
void do_balance ( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "#wYou have #Y%d#w #0bones #win between your #pcheeks#n.\n\r", ch->bones );
    send_to_char( buf, ch );
...
void do_membership( CHAR_DATA *ch, char *argument)
{
  ...
  if (str_cmp(arg, "upgrade"))
  {
    ...
    if (cost > 0)
    {
      sprintf(buf, "It will cost you %d bones to upgrade your membership.\n\r", cost);
      send_to_char(buf,ch);
    }
    return;
  }
  else
  {
    ...
    {
	sprintf( buf, "You can't only do that at a bank.\n\r" );
	send_to_char( buf, ch );
	return;
    }
...
void do_deposit ( CHAR_DATA *ch, char *argument )
{
    ...
    {
        sprintf(buf, "Your bank account is full at %d, place them back between your cheeks.\n\r", allowed );
        send_to_char(buf, ch);
        return;	
    }
    ...
    if((ch->pcdata->bank + amnt) > allowed)
    {
        ...
        sprintf(buf, "You can only hold %d bones in your account, so you only deposit %d.\n\r", allowed, amnt );
        send_to_char(buf, ch);
    	//return;
    }
    ch->pcdata->bank += amnt;
