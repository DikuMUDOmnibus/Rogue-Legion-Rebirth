...
{  
  ...
  if (arg[0] == '\0')
  {
    send_to_char("#wPlease specify which piece of shapeshifter armor you wish to make:\n\r"
  ...
  if (!str_cmp(arg,"dirk")) vnum = 33160;
  else if (!str_cmp(arg,"falchion")) vnum = 33161;
  else if (!str_cmp(arg,"bracer")) vnum = 33162;
  else if (!str_cmp(arg,"amulet")) vnum = 33163;
  else if (!str_cmp(arg,"ring")) vnum = 33164;
  else if (!str_cmp(arg,"plate")) vnum = 33165;
  else if (!str_cmp(arg,"helm")) vnum = 33166;
  else if (!str_cmp(arg,"greaves")) vnum = 33167;
  else if (!str_cmp(arg,"boots")) vnum = 33168;
  else if (!str_cmp(arg,"gauntlets")) vnum = 33169;
  else if (!str_cmp(arg,"sleeves")) vnum = 33170;
  else if (!str_cmp(arg,"cloak")) vnum = 33171;
  else if (!str_cmp(arg,"belt")) vnum = 33172;
  else if (!str_cmp(arg,"shroud")) vnum = 33173;
  else   
  {
    do_shapearmor(ch,"");
    return;
  ...
  obj_to_char(obj, ch);
  ch->practice -= 150;
  act("#wThe #CAIR #waround you transforms into #0$p#w.#n",ch,obj,NULL,TO_CHAR);
