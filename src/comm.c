...
/*    signal(SIGSEGV, iBacktrace); */
...
#if defined(macintosh) || defined(MSDOS)
    boot_db( );
    log_string( "The Rogue Legion is ready to rock." );
    game_loop_mac_msdos( );
#endif
    ...
    sprintf( log_buf, "The Rogue Legion is ready to rock on port %d.", port );
    //wiznet( NULL, WIZ_LOGS, 0, log_buf );
    log_string( log_buf );
...
void close_socket( DESCRIPTOR_DATA *dclose )
{
  ...
  if ((ch = dclose->character) != NULL)
  {
    ...
    //wiznet( ch, WIZ_LINKS, get_trust(ch), log_buf );
    log_string( log_buf );   
...
bool read_from_descriptor( DESCRIPTOR_DATA *d )
{
	...
	if (d != NULL && d->character != NULL) {
	    ...
	    //wiznet( NULL, WIZ_DEBUG, 0, log_buf );
            log_string( log_buf );
        }
	...
	else if ( nRead == 0 )
	{
	    ...
	    //wiznet( NULL, WIZ_LINKS, 0, log_buf );
	    return FALSE;
	}
#if !defined(WIN32)
...
void crashrecov(int iSignal)
{
  ...
  execl(EXE_FILE, "mcloud", buf, "crashrecov", buf2, (char *) NULL);
...
if (IS_SET(ch->act, PLR_BRIEF5))
	...
	else if ( IS_SET(ch->act, PLR_PROMPT) )
	{
	    ...
	    {
		sprintf(exp_str, "%s", commanum(ch->exp));
		COL_SCALE(exp_str, ch, ch->exp, 1000);
	        sprintf( buf, "#n<[#C%sX#n] [#C?#bH #C?#bM #C?#bV#n]> ",exp_str );
	    }
		...
		{
		    sprintf(cond, "#0[#Y***  #0]#n");
		}
		else if ((victim->hit*100/victim->max_hit) < 100)
		{
		    sprintf(cond, "#0[#g**** #0]#n");
		}
		else if ((victim->hit*100/victim->max_hit) >= 100)
		{
		    sprintf(cond, "#0[#G*****#0]#n");
		}
		sprintf(hit_str, "%s", commanum(ch->hit));
		COL_SCALE(hit_str, ch, ch->hit, ch->max_hit);
		sprintf(mana_str, "%s", commanum(ch->mana));
		COL_SCALE(mana_str, ch, ch->mana, ch->max_mana);
		sprintf(move_str, "%s", commanum(ch->move));
		COL_SCALE(move_str, ch, ch->move, ch->max_move);
		sprintf( buf, "#0-=#n%s#0=- -=#n%s#Rhp #n%s#Rm #n%s#Rmv#0=- #n",
		...
		sprintf(exp_str, "%s", commanum(ch->exp));
		COL_SCALE(exp_str, ch, ch->exp, 1000);
	        sprintf( buf, "#0-=#n%s#Rxp#0=- -=#n%s#Rhp #n%s#Rm #n%s#Rmv#0=- #n",
    ...
    for (i = 0; i < length ; i++)
    {
        if (txt[i] == '#' && txt[i+1] != 'I' && txt[i+1] != 'N')               
        {
            ...
            switch (txt[i])
            {
                ...
                case 'f': tb[j++] = 27; tb[j++] = '['; tb[j++] = '5'; tb[j++] ='m'; break;
                ...
                case 'N':
                case 'n':
                    tb[j++] = 27; tb[j++] = '['; tb[j++] = '0'; tb[j++] ='m'; break;
  ...
  *ptr = '\0';
  ...
  length = strlen(output);
    ...
    if ( length >= MAX_STRING_LENGTH )
    {
            ...
            d->outbuf           = alloc_mem( d->outsize );   
            d->outtop       = 2;
            sprintf(output,"Too much text to show, clearing....");
            length=strlen(output);
            logf("Too much text to show, clearing..... (%s)",d->character->name);
    }
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
	    ...
	    sprintf(kav,ch->pcdata->switchname);
            ...
            if (IS_SET(ch->act, PLR_DENY))
            {
              if (ch->pcdata->denied < current_time)
              {
                ...
                //wiznet( ch, WIZ_MISC, get_trust(ch), log_buf );
                log_string( log_buf);
              }
            }
	}
	...
	else
	{
  	  ...
  	  if (!ragnarok) 
  	  {
  	  	if (ch->class == CLASS_NINJA) sprintf(buf, "#R%s #0steps out of the shadows.#n",ch->name);
           else if (ch->class == CLASS_MAGE) sprintf(buf, "#R%s #7gates into the realm.#n",ch->name); 
           else if (ch->class == CLASS_GHOUL) sprintf(buf, "#c%s #Gstaggers in, hungry for human flesh.#n",ch->name);
           else if (ch->class == CLASS_SKYBLADE) sprintf(buf, "#L%s #Cleaps down from the #Wclouds#C.#n",ch->name);
           else if (ch->class == CLASS_DEMON) sprintf(buf, "#r%s #Rreturns, ready to capture more #ysouls!#n",ch->name);
           else if (ch->class == CLASS_LICH) sprintf(buf, "#0%s #Pappears in an explosion of #Cm#Pa#pg#Pi#Cc!#n",ch->name);
           else if (ch->class == CLASS_VAMPIRE) sprintf(buf, "#g%s #Gstalks in, eyes #Rbu#yrni#Rng #Gwith #rb#Rloodlus#rt!#n",ch->name);
           else if (ch->class == CLASS_SAMURAI) sprintf(buf, "#B%s #cbrandishes their swords, awaiting another battle!#n",ch->name);
           else if (ch->class == CLASS_ANGEL) sprintf(buf, "#W%s #Yfloats down from #CHeaven.#n",ch->name);
           else if (ch->class == CLASS_SHAPESHIFTER) sprintf(buf, "#l%s #oshifts into being from nowhere.#n",ch->name);
           else if (ch->class == CLASS_MONK) sprintf(buf, "#r%s #breturns to complete their #wmartial arts #bstudies.#n",ch->name);
           else if (ch->class == CLASS_DROID) sprintf(buf, "#0%s #woils up their joints and returns for battle.#n",ch->name);
           else if (ch->class == CLASS_TANARRI) sprintf(buf, "#Y%s #RRises from hell looking for fresh meat.#n",ch->name);
           else if (ch->class == CLASS_CYBORG) sprintf(buf, "#0%s #wis done charging their batteries and returns for battle.#n",ch->name);
           else if (ch->class == CLASS_THIEF) sprintf(buf, "#0%s #wsteps from the shadows picking their teeth with a dagger.#n",ch->name);
	   else 
	   {
	   	 switch(number_range(0,5))
            {
            default: sprintf(buf,"#w%s becomes part of the collective", ch->name); break;
            case 0: sprintf(buf,"#w%s grabs their sword and prepares for battle.",ch->name); break;
            case 1: sprintf(buf,"#wYou hear a loud #Y-=THUD=- #was %s is dropped from the #Csky.#n",ch->name);break;
            case 2: sprintf(buf,"#wYou notice %s picking their nose as they enter the game.",ch->name);break;
            case 3: sprintf(buf,"#w%s trips over the '#RWelcome#w' sign, while entering.",ch->name); break;
            case 4: sprintf(buf,"#wWhile entering, %s gets wacked over the head with a bat.",ch->name);break;
            case 5: sprintf(buf,"#0%s joins the collective'.#n",ch->name);break;
            }
	   	 //sprintf(buf,"#G%s #wenters #R%s.#n",ch->name, MUDNAME);
	   	}
	      enter_info(buf);
  	  }
          else 
          {
          	sprintf(buf,"#G%s #wenters #R%s #Y(#0Ragnarok#Y).#n",ch->name, MUDNAME);
                enter_info(buf);
          }
	}
        if(ch->pcdata->bounty > 0 && !IS_IMMORTAL(ch))
        {
        	sprintf(buf, "#W%s #0has a bounty of #Y%d #0bones on their #RHead#0.#n", ch->name, ch->pcdata->bounty);
        	enter_info(buf);
        }
	ch->fight_timer = 0;
	if(ch->pcdata->quest > 0)
	{
		sprintf(buf, "#RYour #W%d #RQP has been converted to #W%d #yBONES#n\n\r", ch->pcdata->quest, ch->pcdata->quest * 5);
		stc(buf, ch);
		ch->bones += ch->pcdata->quest * 5;
		ch->pcdata->quest = 0;
	}
    ...
    if(i > 0)
    {
    	sprintf(buf, "#R+++ #WThere were #R#f%d #n#Wchanges made to the mud today type #GCHANGES#W to view #R+++#n\n\r", i);
    	stc(buf, ch);
    }
	if ( !IS_NPC(ch) )
       {
  ...
  if ( strcmp( crypt ( argument, ch->pcdata->pwd ), ch->pcdata->pwd ) )
    ...
    case CON_READ_MOTD:
	write_to_buffer( d,     "\n\r#0**#WWelcome to #Ra mc clone mud#0**\n\r\n\r#0**#WPut your slogan here#0**#n\n\r", 0 );
	ch->next	= char_list;
	char_list	= ch;
	d->connected	= CON_PLAYING;
	...
	sprintf(buf,"#0A #RNEW #0victim#w named #G%s #0is born to #W%s.#n", ch->name, MUDNAME );
	enter_info(buf);
	ch->fight_timer = 0;
...
bool check_reconnect( DESCRIPTOR_DATA *d, char *name, bool fConn )
{
	...
	&&   !str_cmp( GET_PC_NAME(d->character), GET_PC_NAME(ch) ) )
	{
		...
		if(ch->tells)
		{
		    sprintf( buf, "Reconnecting.  You have #R%d#n tells waiting.\n\r",
			ch->tells );
		    send_to_char( buf, ch );
...
{
send_to_char("#w-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-\r\n",ch);
}

void divide5_to_char(CHAR_DATA *ch)
{
cent_to_char("#w-=[#b***********#w]=-------------=[#b***********#w]=-#n",ch);
}

void divide6_to_char(CHAR_DATA *ch)
{
cent_to_char("#0-    -   -  - - -#w- ---====#W*#w====--- -#0- - -  -   - -",ch); }
...
if (ln > 16)
{
	sprintf(buf, "#w-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[                     #w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-=[#0**#w]=-#n"); 
	wdth = (17 - ln) / 2 + 20;
}
else
{
   ...
   while( *str != '\0' )
   {
      ...
      case 'X' :
        sprintf( buf2, "\n\r");
            i = buf2; break;
         case 'q' :
            sprintf( buf2, "%d", ch->bones                     );
            ADD_COLOUR(ch, buf2, L_CYAN);
            i = buf2; break;
      case 'Q' :
        if (!IS_NPC(ch))
          sprintf(buf2, "%d", ch->pcdata->stats[SHADOW_POWER]);
        else sprintf(buf2, " ");
        i = buf2; break;
         case 'f' :
	       ...
	       sprintf( buf2, "N/A" );
            ...
            else
            {
               ...
               else if ((victim->hit*100/victim->max_hit) < 35)
               {sprintf(buf2, "Hurting");ADD_COLOUR(ch, buf2, D_RED);}
               else if ((victim->hit*100/victim->max_hit) < 50)
               {sprintf(buf2, "Poor");ADD_COLOUR(ch, buf2, D_RED);}
               else if ((victim->hit*100/victim->max_hit) < 75)
               {sprintf(buf2, "Wounded");ADD_COLOUR(ch, buf2, BROWN);}
               else if ((victim->hit*100/victim->max_hit) < 85)
               {sprintf(buf2, "Bruised");ADD_COLOUR(ch, buf2, YELLOW);}
               else if ((victim->hit*100/victim->max_hit) < 100)
               {sprintf(buf2, "Good");ADD_COLOUR(ch, buf2, D_GREEN);}
               else if ((victim->hit*100/victim->max_hit) >= 100)
               {sprintf(buf2, "Perfect");ADD_COLOUR(ch, buf2, L_GREEN);}
            }
            i = buf2; break;
         case 'F' :
            if ( ( victim = ch->fighting ) == NULL )
       ...
       case 'c' :
         ...
         else if (IS_CLASS(ch, CLASS_SHADOW) && !IS_NPC(ch))
         {
       ...
       case 'Y':  // not used yet
         ...
         if (!IS_NPC(ch) && (IS_CLASS(ch, CLASS_WEREWOLF) ||
              IS_CLASS(ch, CLASS_VAMPIRE) || IS_CLASS(ch, CLASS_SHADOW) ||
 ...
 */
void pager_to_char ( const char *txt, CHAR_DATA * ch )
{
	if(IS_NPC(ch))
	{
    send_to_char ( txt, ch );
