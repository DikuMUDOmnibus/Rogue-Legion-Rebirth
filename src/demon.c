...
void do_damn( CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_DEMPOWER( ch, DEM_DAMN) && IS_CLASS(ch,CLASS_DEMON))
        {
	    send_to_char("You haven't been granted the gift of damning.\n\r",ch);
	    return;
        }

    if (!TIME_UP(ch, TIMER_DAMN)
	&& ch->level < 12)
    {
	send_to_char("This ability can only be used once every 3 hours.\n\r", ch);
	return;
    }
...
void do_demonarmour( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->practice < 100)
    {
     send_to_char("It costs 100 points of primal to create a piece of demon armour.\n\r",ch);
     return;
    }
