...
void do_realms( CHAR_DATA *ch, char *argument )
{
    ...
    {
	sprintf(buf,"#cRealms of Power#n: #GS#chadow#n #g(#G%d#g)#n, #GZ#combie#n #g(#G%d#g), #GU#cndead #GM#cight#n #g(#G%d#g).#n\n\r",
	...
	{
	    sprintf(buf,"#cYou have already gained all the powers of the #0%s #crealms.#n\n\r", arg1);
	    send_to_char(buf,ch);
	    return;
	}
	if ( cost > ch->pcdata->stats[GHOUL_PTS] )
	{
	    sprintf(buf,"#cYou need #G%d #cGhoul Points to manifest in the #0%s #crealm.#n\n\r", cost, arg1);
	    send_to_char(buf,ch);
	    return;
	}
	ch->pcdata->powers[improve] += 1;
	ch->pcdata->stats[GHOUL_PTS] -= cost;
	sprintf(buf,"#cYou manifest deeper into the #0%s #crealm!.#n\n\r", arg1);
	send_to_char(buf,ch);
    }
