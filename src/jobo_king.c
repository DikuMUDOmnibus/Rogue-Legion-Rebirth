...
void do_kingdoms(CHAR_DATA *ch, char *argument)
{
  ...
  sprintf(buf, "Name          Pkills  Pdeaths  Leader   REQS : hps    move    mana     bones\n\r");
  for (i = 1; i <= MAX_KINGDOM; i++)
  {
    /*
     * calculation ratio
...
void do_kdeposit(CHAR_DATA *ch, char *argument) {
	...
	if ( amt > ch->bones)
		...
		sprintf(buf,"You deposit %d bones into the kingdom treasury.\n\r",amt);
		send_to_char(buf,ch);
		sprintf(buf,"%s has deposited %d bones into the kingdom treasury.",ch->name,amt);
		do_info(ch,buf);
...
void do_warlist(CHAR_DATA *ch, char *argument)
{
	...
	{
	sprintf(buf, "#R(#W%d#R) #n%s's Kingdom of the %s is at war with\n\r%s's Kingdom of the %s\n\r",
