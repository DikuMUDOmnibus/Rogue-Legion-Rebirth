...
void do_displacerform( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You need a Death Symbiote for this.\n\r",ch);
	return;
    }
    if (ch->symbiotelvl < 90)
    {
	send_to_char("You need your Death Symbiote to level 60 for this.\n\r",ch);
	return;
    }
    if (IS_SET(ch->affected_by, AFF_POLYMORPH))
    {   
      	act( "You shift into human form.", ch, NULL, NULL, TO_CHAR);
    	act( "$n's shifts into human.", ch, NULL, NULL, TO_ROOM );
	REMOVE_BIT(ch->symbioteaff, SYMBIOTE_DISPLACER);
	...
	if (ch->mounted == IS_RIDING) do_dismount(ch,"");
      	act( "You shift into a displacer beast.", ch, NULL, NULL, TO_CHAR);
    	act( "$n's shifts into a displacer beast.", ch, NULL, NULL, TO_ROOM );
	SET_BIT(ch->symbioteaff, SYMBIOTE_DISPLACER);
    	SET_BIT(ch->affected_by, AFF_POLYMORPH);
    	ch->hitroll += 400;
    	ch->damroll += 400;
    	ch->armor   -= 200;
    	sprintf(buf, "%s The #0D#7i#0splac#7e#0r #GB#geas#Gt#n", ch->name);
    	free_string(ch->morph);
...
void improve_str(CHAR_DATA * ch)
{
  ...
  if (ch->pcdata->perm_str >=100) return;
...
void improve_dex(CHAR_DATA * ch)
{
  ...
  if (ch->pcdata->perm_dex >=100) return;
  if (dice1 >= 98 || dice2 >= 99)
   {
...
void improve_con(CHAR_DATA * ch)
{
  ...
  if (ch->pcdata->perm_con >=100) return;
  if (dice1 >= 98 || dice2 >= 99)
   {
...
send_to_char("#R[#0--------------------#R[#7Symbiote Screen#R]#0--------------------#R]#n\n\r", ch);
    ...
    send_to_char("Level 25: Bark Skin\n\r",ch);
        if (ch->symbiotetype == 1)
    send_to_char("Level 50: Mana Transfer\n\r",ch);
    ...
    send_to_char("Level 80: Holy Beam\n\r",ch);
        if (ch->symbiotetype == 2)
    send_to_char("Level 90: Displacer form\n\r",ch);
...
void do_consecrate(CHAR_DATA *ch, char *argument)
{
	...
	send_to_char( buf, ch );
	sprintf( buf, "Shocking(5), Flaming(5), Frost(5), Vampiric(5), Poison(5)\n\r",ch);
	send_to_char( buf, ch );
	...
	send_to_char("Cost of Min Damage Increase: 1 Cost of Max Damage Increase: 1 \n\r", ch);
		if (IS_SET(obj->weapflags, WEAPON_FLAMING))
		{
		send_to_char("Your weapon is flaming.\n\r",ch);
		}
		if (IS_SET(obj->weapflags, WEAPON_SHOCKING))
		{
		send_to_char("Your weapon is Shocking.\n\r",ch);
		}
		if (IS_SET(obj->weapflags, WEAPON_FROST))
		{
		send_to_char("Your weapon has Frost.\n\r",ch);
		}
		if (IS_SET(obj->weapflags, WEAPON_VAMPIRIC))
		{
		send_to_char("Your weapon is Vampiric.\n\r",ch);
		}
		if (IS_SET(obj->weapflags, WEAPON_POISON))
		{
		send_to_char("Your weapon is Poisoned.\n\r",ch);
		}
	}
	if (!str_cmp(arg2,"flaming"))
	{
		...
		if (IS_SET(obj->weapflags, WEAPON_FLAMING))
		{
		send_to_char("Your weapon is already flaming.\n\r",ch);
		return;
		}
		else
		{
	...
	if (!str_cmp(arg2,"shocking"))
	{
		...
		if (IS_SET(obj->weapflags, WEAPON_SHOCKING))
		{
		send_to_char("Your weapon is already Shocking.\n\r",ch);
		return;
		}
		else
		{
		sprintf(buf,"%s is enveloped with Lightning.\n\r",obj->short_descr);
		send_to_char(buf,ch);
	...
	if (!str_cmp(arg2,"frost"))
	{
		...
		if (IS_SET(obj->weapflags, WEAPON_FROST))
		{
		send_to_char("Your weapon already has Frost.\n\r",ch);
		return;
		}
		else
		{
	...
	if (!str_cmp(arg2,"vampiric"))
	{
		...
		if (IS_SET(obj->weapflags, WEAPON_VAMPIRIC))
		{
		send_to_char("Your weapon is already Vampiric.\n\r",ch);
		return;
		}
		else
		{
