...
void do_fbreath(CHAR_DATA *ch, char *argument)
{
 ...
 if (ch->mana < 100 )
  ...
  chance = number_range (1, (100 + ch->pcdata->powers[dragonage]));
...
//  if (chance > 125 && chance <= 128) i = 9;
  ...
  for(i=i;i>0;i--)
   {
     dam = number_range(2000,3500); // 880 * (ch->pRank+ch->pcdata->powers[dragonage]));
...
//  else if (chance > 110 && chance <= 125) i = 8;
  ...
  for(i=i;i>0;i--)
    {
	dam = number_range(2500 , 3000); // 680 * (ch->pRank+ch->pcdata->powers[dragonage]));
	if (dam < 6000) dam =6000;
        dam = cap_dam(ch,victim,dam);
...
void do_gbreath(CHAR_DATA *ch, char *argument)
{
   ...
   for(i=i;i>0;i--)
   {
	...
	{
          dam = number_range((ch->damcap[DAM_CAP] - 500), (ch->damcap[DAM_CAP]+(10*125)));
...
//  ch->mana -= 100;
  dam = number_range(2000, 3500); // (5000+(ch->pRank*100)));
   WAIT_STATE (ch, 12);
	...
	if (IS_SET(obj->quest,QUEST_RELIC) || IS_SET(obj->quest,QUEST_ARTIFACT))
	  {
        ...
        if (IS_SET(obj->spectype2, XITEM_BREATH ))
          {
	...
	if (obj->item_type == ITEM_WEAPON)
	  {
		 ...
		 SET_BIT(obj->spectype2, XITEM_BREATH);
		 obj->condition = 100;
		 obj->toughness = 100;
		 obj->resistance = 1;
