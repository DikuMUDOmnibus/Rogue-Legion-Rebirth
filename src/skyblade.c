...
void do_abilities( CHAR_DATA *ch, char *argument )
{
    ...
    {
	sprintf(buf,"Abilities: #LPower (%d), #WMagic (%d), #CSkills (%d).#n\n\r",
...
send_to_char("-=^=-=^=-=^=-=^=-=^=-=^=-=^=-=^=-^-=^=-=^=-=^=-=^=-=^=-=^=-=^=-#n\n\r",ch);*/
   ...
   if (ch->pcdata->powers[SKYBLADE_SKILLS] > 0) {
	...
	sprintf(buf,"Abilities: #LPower (%d), #nMagic (%d), #CSkills (%d).#n\n\r",
	ch->pcdata->powers[SKYBLADE_POWER], ch->pcdata->powers[SKYBLADE_MAGIC],
	ch->pcdata->powers[SKYBLADE_SKILLS]);
	send_to_char(buf,ch);
	return;
	...
	{
	    sprintf(buf,"You have mastered all the abilites in the field of %s.\n\r", arg1);
	    send_to_char(buf,ch);
	    return;
	}
	if ( cost > ch->practice )
	{
	    sprintf(buf,"It costs you %d primal to improve that ability.\n\r", cost);
	    send_to_char(buf,ch);
	    return;
	}
	ch->pcdata->powers[improve] += 1;
	ch->practice -= cost;
	sprintf(buf,"You improve your mastery of %s abilities.\n\r", arg1);
	send_to_char(buf,ch);
    }
...
void do_dragonx (CHAR_DATA *ch, char *argument)
{
  ...
  WAIT_STATE(ch, 12);
  WAIT_STATE(victim, 12);
  sprintf(buf, "#LYou return to the ground and your dragon fades away.#W[%d]\n\r",dam);
  stc(buf,ch);
  sprintf(buf, "#L%s returns to the ground and the dragon fades away.#W[%d]\n\r",ch->name,dam);
  stc(buf, victim);
...
void do_heavenlyrain(CHAR_DATA *ch, char *argument)
{
  ...
  sprintf(buf, "#CYou call upon a heavenly rain to harm your opponent!#W[%d]\n\r",dam);
  stc(buf,ch);
  sprintf(buf, "#C%s calls upon a heavenly rain to harm you!#W[%d]\n\r",ch->name,dam);
  stc(buf,victim);
