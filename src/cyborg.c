...
void do_implant( CHAR_DATA *ch, char *argument )
{
  ...
  {
    sprintf(buf,"#BCurrent implant levels#R: #wFace: #R%d#w, Legs: #R%d#w, Body: #R%d#w.#n\n\r\n\r",
  ...
  if (arg2[0] == '\0')
  {
    ...
    else if (!str_cmp(arg1,"legs"))
    {
      ...
      send_to_char("#wYour spider legs have been replaced with stronger mechanical ones.#n\n\r",ch);
      if (ch->pcdata->powers[CYBORG_LIMBS] > 1)
      send_to_char("#wLloth has blessed you with superior speed.#n\n\r",ch);
      if (ch->pcdata->powers[CYBORG_LIMBS] > 2)
      send_to_char("#wYour are one of Lloths champions.#n\n\r",ch);
      if (ch->pcdata->powers[CYBORG_LIMBS] > 3)
      send_to_char("#wYou are a true drow warrior, you have supreme fighting skills.#n\n\r",ch);
      if (ch->pcdata->powers[CYBORG_LIMBS] > 4)
      send_to_char("#wYour spit is now pure poison.#n\n\r",ch);
      return;
    }
    ...
    sprintf(buf,"#BCurrent implant levels#w: #wFace: #R%d#w, Legs: #R%d#w, Body: #R%d#w.#x\n\r\n\r",
    UMIN(ch->pcdata->powers[CYBORG_FACE],5),   
    UMIN(ch->pcdata->powers[CYBORG_LIMBS],5),
    UMIN(ch->pcdata->powers[CYBORG_BODY],6));  
    send_to_char(buf,ch);
  ...
  if (!str_cmp(arg2,"improve"))
  {
      ...
      {
        sprintf(buf,"Your bionic power must be greater than %d before you can improve your %s implants.\n\r",cost,arg1);
        send_to_char(buf,ch);
        return;
      }
...
void do_cubeform( CHAR_DATA *ch, char *argument )
{
  ...
  {
    act( "$n eyes glow with red light as a bloodlust fills them.",ch,NULL,NULL,TO_ROOM);
    send_to_char("You become a true TERMINATOR!.\n\r",ch);
    SET_BIT(ch->newbits, NEW_CUBEFORM);
    SET_BIT(ch->affected_by, AFF_POLYMORPH);
    sprintf(buf,"%s the Terminator",ch->name);
    free_string(ch->morph);
...
void do_cyborgeq( CHAR_DATA *ch, char *argument )
{
    ...
    if (arg[0] == '\0')
    {
    	send_to_char("Please specify which piece of cyborg armor you wish to make: whip ring "
    ...
    if ( ch->practice < 100)
    {
     send_to_char("It costs 100 primal to create a piece of cyborg armour.\n\r",ch);
     return;
    }
