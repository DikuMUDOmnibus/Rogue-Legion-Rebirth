...
void do_runeeq( CHAR_DATA *ch, char *argument )
{
  ...
  if (arg[0] == '\0')
  {
    send_to_char("Please specify which piece of rune armor you wish to make: \n\rwater fire earth air\n\r",ch);
    return;
  }

  if ( ch->bones < 10000 )
  {
    send_to_char("You don't have the 10K bones needed.\n\r",ch);
    return;
  }



  if      (!str_cmp(arg,"water"        )) vnum = 82223;
  else if (!str_cmp(arg,"fire"      )) vnum = 82220;
  else if (!str_cmp(arg,"earth"       )) vnum = 82222;
  else if (!str_cmp(arg,"air"      )) vnum = 82221;
  else
  {
    do_runeeq(ch,"");
    return;
  ...
  if(!IS_IMMORTAL(ch)) ch->bones -= 10000;
  obj = create_object(pObjIndex, 50);
  obj->questowner = str_dup(ch->pcdata->switchname);
  obj->ownerid = ch->pcdata->playerid;
  obj_to_char(obj, ch);
  act("You transform the 10K bones into $p.",ch,obj,NULL,TO_CHAR);
  act("$n forms $p out of 10K bones.",ch,obj,NULL,TO_ROOM);
  return;
}
...
void do_mastery(CHAR_DATA *ch, char *argument)
{
  ...
  else if (IS_CLASS(ch, CLASS_SHINOBI)) vnum = 33452;
  else if (IS_CLASS(ch, CLASS_ELEMENTAL)) vnum = 33453;
  else if (IS_CLASS(ch, CLASS_DRAGON)) vnum = 33454;
  else
  {
...
void do_setstance(CHAR_DATA *ch, char *argument)
{
  ...
  if (arg1[0] == '\0')
  {
    ...
    send_to_char(buf,ch);
    sprintf(buf, " #r*#n Minimum/Maximum Cost of Current stance : %-3d/%-3d million exp.                         #r*#n\n\r", min_cost, max_cost);
    send_to_char(buf,ch);
    send_to_char(" #r*****************************************************************************************#n\n\r",ch);
    return;
  }
...
void do_timer( CHAR_DATA *ch, char *argument)
{
  ...
  sprintf(buf, "Bidding on ragnarok starts in   :  %3d minutes.\n\r", ragnaroktime/PULSE_AREA);
    ...
    if (global_exp)
  {
  sprintf(buf, "Double Exp will end in          :  %3d minutes.\n\r", pulse_exp);
    send_to_char(buf,ch);
  }

  if (global_dt)
  {
  sprintf(buf, "Double Training will end in     :  %3d minutes.\n\r", pulse_dt );
    send_to_char(buf,ch);
  }
  
    if (global_cp)
  {
  sprintf(buf, "Double Class Points will end in :  %3d minutes.\n\r", pulse_cp );
    send_to_char(buf,ch);
  }
...
void do_birth(CHAR_DATA *ch, char *argument)
{
  ...
  free_char(victim);
  if (ch->pcdata->genes[4] == SEX_MALE)
    sprintf(buf, "Son of %s and %s.", ch->name, ch->pcdata->cparents);
  else
    sprintf(buf, "Daughter of %s and %s.", ch->name, ch->pcdata->cparents);
...
void do_classportal(CHAR_DATA *ch, char *argument)
{
  ...
  switch(ch->class)
  {
    ...
    //case CLASS_BEASTMASTER:
      // Need something here
      //sprintf(buf1, "You run through the forests, searching for $N.");
      //sprintf(buf2, "$n disappears into the forest.");
      //sprintf(buf3, "You leap out of the forest before $N.");
      //sprintf(buf4, "$n leaps out of from behind a tree.");
      //move = TRUE; 
      //cost = 500;
      //break;
    case CLASS_VAMPIRE:
...
void do_newscore(CHAR_DATA *ch, char *argument)
{
  ...
  sprintf(buf,"  You are currently on Tier #R%d#n\n\r",ch->tier);
    stc(buf,ch);
	
    sprintf(buf,"  You have #Y%d #nTier Kills for this tier#n\n\r",ch->tks);
    stc(buf,ch);
  ...
  if (!IS_NPC(ch))
  {
    sprintf(buf, "  Toughness: %d.%d%%", ch->pcdata->damreduct, ch->pcdata->damreductdec);
    send_to_char(buf,ch);
    sprintf(buf, "  You are a %s#n member of the bank.", 
  ...
  if (!IS_NPC(ch) && IS_CLASS( ch, CLASS_DEMON))
    {
  sprintf(buf, "  You have #C%d#n out of #C%d #npoints of demonic power stored.\n\r",
  ...
  sprintf(buf, "  You have #C%d #nTanar'ri points stored\n\r", ch->pcdata->stats[TPOINTS] );
    ...
    if (IS_CLASS(ch, CLASS_GIANT))
  {
    sprintf(buf, "  Giant Points : #C%d#n\n\r", ch->pcdata->stats[8]);
    send_to_char(buf, ch);
  }
    if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_DEMON))
    {
	...
	sprintf( buf, "  You have #C%d#n drow power points stored\n\r",
	ch->pcdata->stats[DROW_POWER] );
	send_to_char( buf, ch );
	sprintf( buf, "  You have #C%d #npoints of magic resistance\n\r",
    ...
    {
      sprintf(buf,"  Your shapeshifter counter is : #C%d#n\n\r",ch->pcdata->powers[SHAPE_COUNTER]);
      stc(buf,ch);
      sprintf(buf,"  Your phase counter is        : #C%d#n\n\r",ch->pcdata->powers[PHASE_COUNTER]);
      stc(buf,ch);
    }

    if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_ANGEL))
    {
      sprintf(buf,"  Angelic Justice   : #C%d#n\n\r",ch->pcdata->powers[ANGEL_JUSTICE]);
      stc(buf,ch);
      sprintf(buf,"  Angelic Love      : #C%d#n\n\r",ch->pcdata->powers[ANGEL_LOVE]);
      stc(buf,ch);
      sprintf(buf,"  Angelic Harmony   : #C%d#n\n\r",ch->pcdata->powers[ANGEL_HARMONY]);
      stc(buf,ch);
      sprintf(buf,"  Angelic Peace     : #C%d#n\n\r",ch->pcdata->powers[ANGEL_PEACE]);
      stc(buf,ch);
   }

    if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_VAMPIRE))
    {
      sprintf(buf,"  Your current beast is : #C%d#n\n\r",ch->beast);
      stc(buf,ch);

    }
    if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_MONK) )
    {
      sprintf(buf,"  Your block counter is currently: #C%d#n\n\r",ch->monkblock);
      stc(buf,ch);
      sprintf(buf,"  Your current level of chi:       #C%d#n\n\r",ch->chi[CURRENT]);
      stc(buf,ch);
      sprintf(buf,"  Your maximum level of chi:       #C%d#n\n\r",ch->chi[MAXIMUM]);
      stc(buf,ch);
    }
    
    if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_SAMURAI) )
    {
      sprintf(buf,"  Your current focus points:       #C%d#n\n\r",ch->focus[CURRENT]);
      stc(buf,ch);
      sprintf(buf,"  Your maximum focus points:       #C%d#n\n\r",ch->focus[MAXIMUM]);
      stc(buf,ch);
    }
    
    if( !IS_NPC(ch) && (IS_CLASS(ch, CLASS_WEREWOLF)) && ch->gnosis[GMAXIMUM] > 0 )
    {
	sprintf( buf, "  Current Gnosis:            #C%d#n\n\r", ch->gnosis[GCURRENT] );
	stc( buf, ch);
	sprintf( buf, "  Maximum Gnosis:            #C%d#n\n\r", ch->gnosis[GMAXIMUM] );
	stc( buf, ch);
    }
	...
	{
	    sprintf(buf,"  You have attained #C%d #npoints of silver tolerance\n\r",ch->siltol);
	    send_to_char( buf, ch );
	}
	if ( IS_CLASS(ch,CLASS_VAMPIRE) && ch->rage > 0)
	{
	    sprintf(buf,"  The beast is in control of your actions:  Affects Hitroll and Damroll by #C+%d\n\r",ch->rage);
	    send_to_char( buf, ch );
	}
	if ( (IS_CLASS(ch, CLASS_WEREWOLF)) && IS_SET(ch->special,SPC_WOLFMAN) && ch->rage > 0)
	{
	    sprintf(buf,"  You are raging:  Affects Hitroll and Damroll by #C+%d\n\r",ch->rage);
	    send_to_char( buf, ch );
	}
	if (IS_CLASS(ch, CLASS_NINJA) && ch->rage > 0) {
	sprintf(buf,"  Your michi gives you an extra #C%d#n hitroll and damroll\n\r", ch->rage);
	send_to_char(buf,ch);
	}
	if (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_CHIKYU] >= 6
	&& ch->pcdata->powers[HARA_KIRI] > 0) {
	sprintf(buf,"  You receive the power of HaraKiri for #C%d#n more ticks\n\r",ch->pcdata->powers[HARA_KIRI]);
	send_to_char(buf,ch);
	}

        if ( !IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_VAMPIRE) && ch->rage > 0 && !IS_CLASS(ch, CLASS_NINJA))
	{
	   sprintf(buf,"  You are in a mad frenzy, adding #C%d#n Hitroll and Damroll\n\r",ch->rage);
	   stc(buf, ch);
	}

	else if ( IS_CLASS( ch, CLASS_DEMON) && ch->pcdata->stats[DEMON_POWER] > 0)
	{
	    sprintf(buf,"  You are wearing demonic armour:  Affects Hitroll and Damroll by #C+%d\n\r",
		((ch->pcdata->stats[DEMON_POWER]) *ch->pcdata->stats[DEMON_POWER]));
	    send_to_char( buf, ch );
	}
        else if ( IS_CLASS( ch, CLASS_DROW) && ch->pcdata->stats[DEMON_POWER] > 0)
        {
            sprintf(buf,"  You are wearing drow armour:  Affects Hitroll and Damroll by #C+%d\n\r",
                ((ch->pcdata->stats[DEMON_POWER]) *ch->pcdata->stats[DEMON_POWER]));  
            send_to_char( buf, ch );
        }
	...
	if (ch->fight_timer > 0) {
	sprintf(buf,"  You have #C%d#n rounds left on your fight timer#n\n\r", ch->fight_timer);
	send_to_char(buf,ch);
  }
      if ( IS_IMMORTAL ( ch ) )
    {
  	 ...
  	 send_to_char("\n\r", ch);
    	 sprintf(buf, "  #nInvis: #C%3d#n    Incog Level: #C%d#n    Ghost Level    #C%3d#n\n\r",
