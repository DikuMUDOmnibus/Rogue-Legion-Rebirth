...
void do_drowcreate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->practice < 100)
    {
	send_to_char("It costs 100 points of primal to create a piece of drow armour.\n\r",ch);
	return;
    }
...
void do_shadowwalk(CHAR_DATA *ch, char *argument)
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch,CLASS_CYBORG) && !IS_CLASS(ch, CLASS_SHADOW))
        {
	send_to_char("Huh?\n\r", ch );
	return;}

    if (!IS_SET(ch->pcdata->powers[SHADOW_POWERS],NSHADOWS_SHADOWWALK) &&  IS_CLASS(ch,CLASS_SHADOW))
    {
        send_to_char("Huh?\n\r", ch );
        return;
    }
...
/*	if (!IS_NPC(victim)) {
	...
	return;}*/
        ...
        if(IS_CLASS(ch, CLASS_CYBORG))
        {
          act("You become line noise and move at lightspeed.", ch, NULL, NULL, TO_CHAR);
          act("$n becomes line noise and moves at lightspeed.", ch, NULL, NULL, TO_ROOM);
        }
        else
        {
