  ...
  switch ( channel )
  {
    default:
      ...
      else if (channel == CHANNEL_CHAT)
      {
        ...
        sprintf( buf, "You %s '#W%s#n'.\n\r", verb, argument );
        send_to_char( buf, ch );
        if (ch->trust > 6)
          sprintf( buf, "#Y(#G*#Y)#C%s#Y(#G*#Y)#n '#1$t#n'.", ch->name);
        else if (IS_NPC(ch))
          sprintf( buf, "%s chats '#W$t#n'.", ch->short_descr);
        else if (IS_CLASS(ch, CLASS_WEREWOLF))
          sprintf( buf, "%s barks '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_MAGE))
          sprintf( buf, "%s chants '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_SHAPESHIFTER))
          sprintf( buf, "%s whispers '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_VAMPIRE))
          sprintf( buf, "%s snarls '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_DRONE))
          sprintf( buf, "%s drools '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_ANGEL))
          sprintf( buf, "%s preaches '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_TANARRI))
          sprintf( buf, "%s booms '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_LICH))
          sprintf( buf, "%s squicks '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_HOBBIT))
          sprintf( buf, "%s burps '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_DEMON))
          sprintf( buf, "%s growls '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_CYBORG))
          sprintf( buf, "%s chitters '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_GIANT))
          sprintf( buf, "%s rumbles '#W$t#n'.",ch->name);
        else if (IS_CLASS(ch, CLASS_SKYBLADE))
          sprintf( buf, "%s hollars '#W$t#n'.",ch->name);
        else  
          sprintf( buf, "%s %ss '#W$t#n'.", ch->name, verb );
      }
      else
      {
        sprintf( buf, "You %s '#W%s#n'.\n\r", verb, argument );
    ...
    case CHANNEL_CLASS:
      switch(sub_channel)
      {
        ...
        case CC_DRONE:
          ...
          break;
        case CC_SKYBLADE:
          sprintf( buf, "#B=#R*#w>#0%s#w<#R*#B= #W'$t#W'.#n", ch->name);
          act( buf, ch, argument, NULL, TO_CHAR );
          break;
        case CC_DROW:
