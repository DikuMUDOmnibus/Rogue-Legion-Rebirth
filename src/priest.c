...
void do_godstrike( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->position != POS_FIGHTING )
    {
        ...
        if (!IS_NPC(victim))
           atkdam /= 2;
         sprintf( buf, "Your GodStrike #w-#w-#0-#b*#B*#0*#w(#s MUTILATES #w)#0*#B*#b*#0-#w-#w-#n %s. #B[#w%d#B]#n\n\r",
                 victim->name,
                 atkdam );
         stc( buf, ch );
         sprintf( buf, "%s's GodStrike #w-#w-#0-#b*#B*#0*#w(#s MUTILATES #w)#0*#B*#b*#0-#w-#w-#n you! #B[#w%d#B]#n\n\r",
                 ch->name,
                 atkdam );
         stc( buf, victim );
...
void do_turnundead(CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf,"Undead %s",victim->name);
    sprintf(buf2,"(Undead) %s is here.\n\r",victim->short_descr);
...
void do_pwho( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Fellow Priests online:\n\r");
    send_to_char( buf, ch );
...
return;
        ...
        sprintf(buf, "#0(#C-#C{#W%s#C}#C-#0)#w the holy priest", ch->name);
        free_string( ch->morph );
