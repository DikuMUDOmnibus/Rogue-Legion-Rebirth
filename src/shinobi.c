...
/* Shinobi Class
	...
	Jutsus:
		Shadow- Sneaky Shit
			...
			Butomi - Create illusions of yourself. 1 + Status / 5 clones.
		...
		Fire- Brute Force
			...
			Torra - Burn your opponents hand with a flaming shuriken.
...
void jutsu_torra args ((CHAR_DATA *ch, CHAR_DATA *victim));
...
void do_jutsus(CHAR_DATA *ch, char *argument)
{
	...
	if(move[0] == '\0' && ch->fighting == NULL){
		...
		if(ch->pcdata->powers[S_HANDFIRE] >= 2)
                stc(" #0|       #RTorra #r- Throw a flaming shuriken.                            #0|\n\r",ch);
		...
		if(ch->pcdata->powers[S_RANK] >= S_JOUNIN){
			stc(" #0||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n\r",ch);
			stc(" #0|       #lApprentice - Make another shinobi.                             #0|\n\r",ch);
		}
...
//	dir = is_number(target) ? atoi(target) : -1;
	...
	if(!str_cmp(move,"fagra") && ch->pcdata->powers[S_HANDFIRE] >= 1 && notarg == FALSE && selftarg == FALSE) jutsu_fagra(ch,tarchar);
        ...
        else if(!str_cmp(move,"torra") && ch->pcdata->powers[S_HANDFIRE] >= 2 && notarg == FALSE && selftarg == FALSE) jutsu_torra(ch,tarchar);
	else if(!str_cmp(move,"torra") && ch->pcdata->powers[S_HANDFIRE] >= 2 && (notarg == TRUE || selftarg == TRUE)) JFAIL
...
void do_handmotions(CHAR_DATA *ch, char *argument)
{
	...
	if(IS_NPC(ch)) return;
        ...
        if(!IS_CLASS(ch, CLASS_SHINOBI))return;
...
void do_shinobieq(CHAR_DATA *ch, char *argument)
{
	char arg[MSL];
	const char *bufa;
	char buf[MSL];
	...
	if(ch->practice < 500){
		stc("You need 500 primal to create a shinobi katana.\n\r",ch);
		return;
	}
	if(arg[0] == '\0'){
		stc("You can make:  katana\n\r",ch);
		return;
	}

//	if(!str_cmp(arg,"ring")) vnum = 33288;
//	if(!str_cmp(arg,"pendant")) vnum = 33289;
//	if(!str_cmp(arg,"tunic")) vnum = 33290;
//	if(!str_cmp(arg,"bandana")) vnum = 33291;
//	if(!str_cmp(arg,"greaves")) vnum = 33292;
//	if(!str_cmp(arg,"boots")) vnum = 33293;
//	if(!str_cmp(arg,"gloves")) vnum = 33294;
//	if(!str_cmp(arg,"cloak")) vnum = 33295;
//	if(!str_cmp(arg,"sleeves")) vnum = 33296;
//	if(!str_cmp(arg,"sash")) vnum = 33297;
//	if(!str_cmp(arg,"armguard")) vnum = 33298;
//	if(!str_cmp(arg,"mask")) vnum = 33299;
	if(!str_cmp(arg,"katana")) vnum = 94104;

	if( vnum == 0){
		stc("That is not a peice of eq.\n\r",ch);
		return;
	}
	...
	if(ch->pcdata->powers[S_ELEMENT] == S_FIRE) bufa = "#R";	
	else if(ch->pcdata->powers[S_ELEMENT] == S_EARTH) bufa = "#o";
	else if(ch->pcdata->powers[S_ELEMENT] == S_WIND) bufa = "#C";	
	else if(ch->pcdata->powers[S_ELEMENT] == S_WATER) bufa = "#L";	
	else if(ch->pcdata->powers[S_ELEMENT] == S_HOLY) bufa = "#7";	
	else if(ch->pcdata->powers[S_ELEMENT] == S_SHADOW) bufa = "#0";

	ch->practice -= 500;

	obj = create_object(pObjIndex,50);
	obj->questowner = str_dup(ch->pcdata->switchname);
	sprintf(buf, obj->short_descr, bufa,bufa,bufa,bufa,bufa,bufa);
	free_string(obj->short_descr);
	obj->short_descr = str_dup(buf);
	obj_to_char(obj,ch);
	stc("You create a peice of eq!\n\r",ch);
	return;
...
void do_shinobirank(CHAR_DATA *ch, char *argument)
{
	...
	ch->max_hit >= 50000 && ch->max_move >= 50000 && ch->max_mana >= 50000 &&
	(ch->chakra / 1000) >= 3){
	...
	ch->max_hit >= 60000 && ch->max_move >= 60000 && ch->max_mana >= 60000 &&
	(ch->chakra / 1000) >= 6  && ch->race >= 5){
...
void jutsu_torra(CHAR_DATA *ch, CHAR_DATA *victim)
{

	if(!IS_SET(victim->newbits2,NEW2_TORRA)){
		act("$N's hand is scorched by a flaming shuriken!",ch,NULL,victim,TO_ROOM);
		act("$N's hand is scorched by a flaming shuriken!",ch,NULL,victim,TO_VICT);
		act("$N's hand is scorched by a flaming shuriken!",ch,NULL,victim,TO_CHAR);
		SET_BIT(victim->newbits2,NEW2_TORRA);
	}
	else{
		act("$N's hand cools.",ch,NULL,victim,TO_ROOM);
		act("$N's hand cools.",ch,NULL,victim,TO_CHAR);
		act("$N's hand cools.",ch,NULL,victim,TO_VICT);
		REMOVE_BIT(victim->newbits2,NEW2_TORRA);
	}
	WAIT_STATE(ch,12);
...
//  if( IS_SET( victim->extra, PLR_CASTLE ) ) {
...
void jutsu_butomi(CHAR_DATA *ch)
{
	...
	for(x = 0;x < r; x++){
		ch->pcdata->followers++;
		if(ch->pcdata->powers[S_HANDSHADOW] < 6) victim = create_mobile( get_mob_index( 81101 ) );
		else victim = create_mobile( get_mob_index( 81101 ) );
		...
		if(ch->fighting != NULL) set_fighting(victim,ch->fighting, FALSE);
	}
	...
	ch->pcdata->powers[S_CLONETIMER] = 5;
...
//        return;}
  ...
  act( "$n is swallowed up by the earth.", victim, NULL, NULL, TO_ROOM );
  char_from_room( victim );
  char_to_room( victim, pRoomIndex );
//        if(IS_SET(ch->extra, PLR_CASTLE)) REMOVE_BIT(ch->extra,PLR_CASTLE);
  do_look( victim, "auto" );
  act( "$n appears from the earth.", victim, NULL, NULL, TO_ROOM );
...
//        ch->level = 12;
  	hitdam = 200;
    	armor  = -300;
    	add_affect( obj, APPLY_HITROLL, hitdam );
    	add_affect( obj, APPLY_DAMROLL, hitdam );
...
void jutsu_mian(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	act("$n focus's their holy energy upon you.",ch,NULL,victim,TO_VICT);
        if(IS_CLASS(victim, CLASS_SHINOBI) || (victim->pcdata->powers[S_ELEMENT] = S_HOLY)) 
        {
	amount = 4000 +  ch->race * 1000;
	amount *= ch->pcdata->powers[S_RANK] + 1;
        }
        else if(victim->pcdata->powers[S_ELEMENT] = S_EARTH)
        {
        amount = 3000 +  ch->race * 1000;
        amount *= ch->pcdata->powers[S_RANK] + 1;
        }
        else
        {
        amount = 1000 +  ch->race * 1000;
        amount *= ch->pcdata->powers[S_RANK] + 1;
        }
...
void jutsu_soothe(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	act("$n focus's their soothing energy upon you.",ch,NULL,victim,TO_VICT);
        if(IS_CLASS(victim, CLASS_SHINOBI) || (victim->pcdata->powers[S_ELEMENT] = S_WATER))
        {
        amount = 3000 +  ch->race * 1000;
        amount *= ch->pcdata->powers[S_RANK] + 1;
        }
