...
/* Symbiote Spells and Skills -Xrakisis */

void spell_2xexp( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;

    if (IS_AFFECTED2(victim, AFF2_EXPMOD))
        return;

    if (victim->symbiotetype == 0)
	return;

    af.type      = sn;
    af.duration  = 400 + level;
    af.modifier  = 0;
    af.location  = APPLY_NONE;
    af.bitvector = AFF2_EXPMOD;
    affect_to_char( victim, &af );
    send_to_char( "You start to learn faster.\n\r", victim );
    if ( ch != victim )
        send_to_char( "Ok.\n\r", ch );
    return;
}

void spell_deathray( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;

    if (ch->symbiotetype != 2) return;
    if (ch->symbiotelvl < 80) {
    send_to_char( "You need atleast level 80 for this.\n\r", ch );
	return;}
...
void spell_holybeam( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;

    if (ch->symbiotetype != 1) return;
    if (ch->symbiotelvl < 80) {
    send_to_char( "You need atleast level 80 for this.\n\r", ch );
        return;}
...
void spell_spiritarmor( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    if (ch->symbiotetype == 0) return;
    if (ch->symbiotelvl < 15) {
    send_to_char( "You need atleast level 15 for this.\n\r", ch );
        return;}

    af.type      = sn;
    af.duration  = number_range(250,500);
    af.modifier  = -1000;
    af.location  = APPLY_AC;
    af.bitvector = 0;
    affect_to_char( victim, &af );
    act( "$n is enveloped by #GS#gp#0i#7r#gi#Gt #GA#gr#0m#go#Gr#n.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You are enveloped by #GS#gp#0i#7r#gi#Gt #GA#gr#0m#go#Gr#n.\n\r", victim );
    if ( ch != victim )
        send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_barkskin( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (ch->symbiotetype == 0) return;
    if (ch->symbiotelvl < 25) {
    send_to_char( "You need atleast level 25 for this.\n\r", ch );
        return;}
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
    act( "$n's skin turns to #oB#0a#7r#ok#n.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "Your skin turns to #oB#0a#7r#ok#n.\n\r", victim );
    if ( ch != victim )
        send_to_char( "Ok.\n\r", ch );
    return;
}

void spell_primalrage( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA  *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    
    if (ch->symbiotetype == 0) return;
    if (ch->symbiotelvl < 60) {
    send_to_char( "You need atleast level 60 for this.\n\r", ch );
        return;}
    ...
    if ( ch != victim )
        send_to_char( "Ok.\n\r", ch );
    act( "$n is consumed with primal rage.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You are consumed with primal rage!\n\r", victim );
    if (!IS_NPC(victim)) do_beastlike(victim,"");
    return;
}

void spell_manatransfer( int sn, int level, CHAR_DATA *ch, void *vo )
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;

  if (ch->symbiotetype != 1)
  {
  send_to_char( "You need a Life Symbiote for this.\n\r", victim );
  return;
  }
  if (ch->symbiotelvl < 50)
  {
  send_to_char( "You need a Life Symbiote to Level 50 for this.\n\r", victim );
  return;
  }
  if (ch->mana < 5000)
  {
  send_to_char( "You dont have the required Mana.\n\r", victim );
  return;
  }
  if (ch->hit >= ch->max_hit)
  {
  send_to_char( "Your too healthy.\n\r", victim );
  return;
  }


  send_to_char( "You convert your Mana to Health.\n\r", victim );
  ch->mana -= 5000;
  ch->hit += 5000;
  update_pos( victim );

  if (ch == victim)
    act( "$n Mana Transfers $mself.", ch, NULL, NULL, TO_ROOM );
  else
    act( "$n Mana Transfers $N.", ch, NULL, victim, TO_NOTVICT );
...
/* End of Symbiote Skills and Spells */
