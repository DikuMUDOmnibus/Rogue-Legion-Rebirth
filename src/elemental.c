...
void do_disburse ( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  char buf[MAX_STRING_LENGTH];
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!IS_CLASS(ch, CLASS_ELEMENTAL))
  {
    send_to_char("Only Elementals May Disberse.\n\r",ch);
    return;
  }
  if (ch->move < 600)
  {
...
void do_elementaleq( CHAR_DATA *ch, char *argument )
{
  ...
  if (arg[0] == '\0')
  {
    send_to_char("Please specify which piece of angel armor you wish to make: ring, collarplate, helm, leggings, boots, gloves, sleeves, cloak, belt, bracer, mask, fang.\n\r",ch);
    return;
  }
  if ( ch->practice < 150 )
  {
  ...
  if (!str_cmp(arg,"ring")) vnum = 94042;
  else if (!str_cmp(arg,"collar")) vnum = 94043;
  else if (!str_cmp(arg,"plate")) vnum = 94044;
  else if (!str_cmp(arg,"helm")) vnum = 94045;
  else if (!str_cmp(arg,"leggings")) vnum = 94046;
  else if (!str_cmp(arg,"boots")) vnum = 94047;
  else if (!str_cmp(arg,"gloves")) vnum = 94048;
  else if (!str_cmp(arg,"sleeves")) vnum = 94049;
  else if (!str_cmp(arg,"cloak")) vnum = 94050;
  else if (!str_cmp(arg,"belt")) vnum = 94051;
  else if (!str_cmp(arg,"bracer")) vnum = 94052;
  else if (!str_cmp(arg,"mask")) vnum = 94053;
  else if (!str_cmp(arg,"fang")) vnum = 94054;
  else
  {
    do_elementaleq(ch,"");
    return;
  ...
  {
    send_to_char("Missing object, please inform a Xrakisis or another Coder.\n\r",ch);
    return;
  }
