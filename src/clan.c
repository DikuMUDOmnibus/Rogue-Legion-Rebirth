...
void do_level( CHAR_DATA *ch, char *argument )
{
 ...
 if (IS_CLASS(ch, CLASS_VAMPIRE) )
 {
	...
	sprintf(skill,"#r|#0    Viper #7%-4d    #0Crane #7%-4d    #0Crab #7%-4d    #0Mongoose #7%-4d    #0Bull #7%-4d#r    |\n\r",ch->stance[1],ch->stance[2],ch->stance[3],ch->stance[4],ch->stance[5]);
	stc(skill,ch);
	sprintf(skill,"#r|#0   Mantis #7%-4d   #0Dragon #7%-4d   #0Tiger #7%-4d    #0Swallow  #7%-4d  #0Monkey #7%-4d#r    |\n\r",ch->stance[6],ch->stance[7],ch->stance[8],ch->stance[10],ch->stance[9]);
	stc(skill,ch);
...
void do_oldlevel( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( skill, ch );
    sprintf(skill,"Pizza    : %-4d", ch->stance[16]);
    send_to_char( skill, ch );
    sprintf(skill,"Zarius   : %-4d\n\r", ch->stance[17]);
    send_to_char( skill, ch );

    send_to_char("\n\r---------------------------=[#CSpell Levels#n]=---------------------------------\n\r", ch);
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#PPurple#n : %-4d", ch->spl[0] + 20);
    else sprintf(skill,"#PPurple#n : %-4d", ch->spl[0]);
    send_to_char( skill, ch );
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#BBlue#n : %-4d", ch->spl[2] + 20);
    else sprintf(skill,"#BBlue#n : %-4d", ch->spl[2]);
    send_to_char( skill, ch );
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#RRed#n : %-4d", ch->spl[1] + 20);
    else sprintf(skill,"#RRed#n : %-4d", ch->spl[1]);
    send_to_char( skill, ch );
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#GGreen#n : %-4d", ch->spl[3] + 20);
    else sprintf(skill,"#GGreen#n : %-4d", ch->spl[3]);
    send_to_char( skill, ch );
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#YYellow#n : %-4d\n\r\n", ch->spl[4] + 20);
    else sprintf(skill, "#YYellow#n : %-4d\n\r\n", ch->spl[4]);
    send_to_char( skill, ch );
    ...
    if ( ch->stance[12] == 1 )
      ...
      sprintf(stance, " None" );
      sprintf(skill, "#pAutostance#n:  %s #n\n\r",stance);
     send_to_char(skill, ch);
    if (IS_SET(ch->act, PLR_RIGHTHAND))
	...
	if (ch->generation > 1 && ch->level > 2)
	{
	sprintf(skill,"You are #R%d#W%% #non your way to generation #Y%d.#n\n\r",percent,nextgen);
	//stc(skill,ch);
...
void do_readaura( CHAR_DATA *ch, char *argument )
{
	...
	&& !IS_CLASS(ch, CLASS_MONK) && !IS_CLASS(ch,CLASS_CYBORG) && !IS_CLASS(ch, CLASS_HOBBIT)
...
void do_shield( CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_CLASS(ch, CLASS_VAMPIRE) && !IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_WEREWOLF) &&
      ...
      !IS_CLASS(ch, CLASS_HOBBIT) && !IS_CLASS(ch, CLASS_DRONE))
  {
    send_to_char("Huh?\n\r",ch);
    return;
  }
...
void do_vanish( CHAR_DATA *ch, char *argument )
{
    ...
    if( !IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_VAMPIRE) &&
       !IS_CLASS(ch, CLASS_HOBBIT) && !IS_CLASS(ch, CLASS_FAE) && !IS_CLASS(ch, CLASS_SHADOW))
    {
	send_to_char("Huh?\n\r",ch);
	return;
    }
...
void do_web( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_CYBORG) && !IS_CLASS(ch, CLASS_DROID)
