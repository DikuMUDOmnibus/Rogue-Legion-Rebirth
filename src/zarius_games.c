...
void  do_slots(CHAR_DATA *ch, char *argument )
{
  ...
  if(cost > ch->bones)
    {
      sprintf(buf, "This slot machine costs %d bones to play.\n\r", cost);
      send_to_char(buf, ch);
      return;
    }
  ...
  if(bars == 3)
    {
      ...
      sprintf(buf, "#0|#c{{}#0|#n  %s  %s  %s    |#c{{}#0|#n\n\r", bar_messages[bar1],
    ...
    {
      send_to_char("#0////-----------------------#PSlot Machine#0----------------------\\\\\\\\#n\n\r", ch);
      sprintf(buf, "#0|#c{{}#0|#n  %s  %s  %s  %s  %s  #0|#c{{}#0|#n\n\r", bar_messages[bar1],
  ...
  if(won && wonJackpot)
    {
      sprintf(buf, "You won the jackpot worth %d bones!! The jackpot now stands at %d bones.\n\r",
	      winnings, slotMachine->value[1]);
      send_to_char(buf, ch);
      sprintf(buf, "%s won a jackpot of %d bones from the casino!!!", ch->pcdata->switchname, winnings);
      do_info(ch, buf);
    }
  if(won && !wonJackpot)
    {
      sprintf(buf, "You matched %d bars and won %d bones! The jackpot is now worth %d bones.\n\r",
	      numberMatched, winnings, slotMachine->value[1]);
      send_to_char(buf, ch);
    }
  if(!won)
    {
      sprintf(buf, "Sorry you didn't win anything. The jackpot is now worth %d bones.\n\r",
	      slotMachine->value[1]);
      send_to_char(buf, ch);
    }
...
void do_high_dice( CHAR_DATA *ch, char *argument)
{
  ...
  sprintf(buf, "#c%s#w rolled two dice with a total of #Y%d!#n\n\r", dealer->short_descr, 
	  dealerDice);
  send_to_char(buf, ch);
  sprintf(buf, "#wYou rolled two dice with a total of #Y%d!#n\n\r", playerDice);
  send_to_char(buf, ch);

  if(dealerDice > playerDice)
    {
      sprintf(buf, "#RYou lost! #c%s#0 takes your bet of #Y%d bones#0.#n\n\r",
  ...
  if(dealerDice < playerDice)
    {
      sprintf(buf, "#RYou won! #c%s #0gives you your winnings of #Y%d bones#0.#n\n\r",
	      dealer->short_descr, bet);
      send_to_char(buf, ch);
      ch->bones += bet;
    }
  ...
  if(dealerDice == playerDice)
    {
      sprintf(buf, "#RYou lost! #0The dealer always wins in a tie. You lose #Y%d bones#0.#n\n\r",
	      bet);
      send_to_char(buf, ch);
      ch->bones -= bet;
    }
