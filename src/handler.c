...
int get_curr_str( CHAR_DATA *ch )
{
    ...
    max = 110;
    if(IS_CLASS(ch, CLASS_SAMURAI) || IS_IMMORTAL(ch) ) max = 120;
...
int apply_ac( OBJ_DATA *obj, int iWear )
{
    ...
    case WEAR_SPECIAL:	return     obj->value[0];
    case WEAR_BODYART:	return     obj->value[0];
    case WEAR_MEDAL:	return     obj->value[0];
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    if (!ch->shop_fun)
  {
	...
	return;
        ...
        if ((obj->pIndexData->vnum >= 33260 && obj->pIndexData->vnum <= 33261)
         && (IS_NPC(ch) || !IS_CLASS(ch,CLASS_FAE))) {
        act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR);
        act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM);
        obj_from_char(obj);
        obj_to_room(obj, ch->in_room);
        ...
        if ((obj->pIndexData->vnum >= 33300 && obj->pIndexData->vnum <= 33313)
         && (IS_NPC(ch) || !IS_CLASS(ch,CLASS_DRONE))) {
        act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR);
        act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM);
        obj_from_char(obj);
        obj_to_room(obj, ch->in_room);
        ...
        if ((obj->pIndexData->vnum >= 33240 && obj->pIndexData->vnum <= 33255)
         && (IS_NPC(ch) || !IS_CLASS(ch,CLASS_HOBBIT))) {
        act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR);
        act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM);
        obj_from_char(obj);
        obj_to_room(obj, ch->in_room);
        ...
        if ((obj->pIndexData->vnum >= 33280 && obj->pIndexData->vnum <= 33293)
         && (IS_NPC(ch) || !IS_CLASS(ch,CLASS_GIANT))) {
        act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR);
        act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM);
        obj_from_char(obj);
        obj_to_room(obj, ch->in_room);
   ...
   // }
	...
	else if (sn == 16)
	{
	    ...
	    act("$n starts regenerating at a fast rate.",ch,NULL,NULL,TO_ROOM);
	}
	else if (sn == 17)
	{
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, ROOM_INDEX_DATA *room, char *argument )
{
	...
	else if ( get_trust ( ch ) < rch->ghost_level ) continue;
	else if ( !IS_NPC(rch) && IS_EXTRA(rch, EXTRA_OSWITCH) ) continue;
	else if ( (ch && !can_see( ch, rch )) || ( !is_name( arg, rch->name ) &&
...
CHAR_DATA * get_char_world( CHAR_DATA *ch, char *argument )
{
    ...
    else if (( ch && !can_see( ch, wch ) ) || (!is_name(arg, wch->name) && (IS_NPC(wch) || !is_name(arg, wch->pcdata->switchname)) && (IS_NPC(wch) || !is_name(arg, wch->morph))))
      continue;
    if (++count == number)
      return wch;
  }
...
char *item_type_name( OBJ_DATA *obj )
{
    ...
    case ITEM_HOLYSYMBOL:	return "holysymbol";
    case ITEM_FAITHTOKEN:       return "faith token";
    case ITEM_FIRESTONE: 	return "firestone";
    case ITEM_EARTHSTONE:	return "earthstone";
    case ITEM_AIRSTONE:		return "airstone";
    case ITEM_WATERSTONE:	return "waterstone";
}
...
char *affect2_bit_name( int vector )
{
    ...
    buf[0] = '\0';
    if ( vector & AFF2_THORNS        ) strcat( buf, " brambles"      );
    if ( vector & AFF2_DARMOR        ) strcat( buf, " divine_armor"  );
    if ( vector & AFF2_FARMOR        ) strcat( buf, " faith armor"   );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *quest_bit_name( int vector )
{
    ...
    buf[0] = '\0';
    if ( vector & QUEST_RELIC             ) strcat( buf, " relic"      );
    if ( vector & QUEST_ARTIFACT          ) strcat( buf, " artifact"  );
    if ( vector & QUEST_SPELLPROOF        ) strcat( buf, " spellproof armor"   );
    if ( vector & QUEST_HOLYSYMBOL        ) strcat( buf, " holy_symbol"   );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
