...
bool aedit_show( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "#wName:     #0[#W%5d#0] #B%s#n\n\r", pArea->vnum, pArea->name );
    send_to_char( buf, ch );

    sprintf( buf, "#wFile:     #W%s#n\n\r", pArea->filename );
    send_to_char( buf, ch );

    sprintf( buf, "#wVnums:    #0[#W%d#R-#W%d#0]#n\n\r", pArea->lvnum, pArea->uvnum );
    send_to_char( buf, ch );

  sprintf(buf, "#wCvnum:    #0[#W%d#0]#n\n\r", pArea->cvnum);
    send_to_char( buf, ch );

    sprintf( buf, "#wPlayers:  #0[#W%d#0]#n\n\r", pArea->nplayer );
    send_to_char( buf, ch );

  sprintf(buf, "#wAge:      #0[#W%d#0]#n\n\r", pArea->age);
    send_to_char( buf, ch );

    sprintf( buf, "#wSecurity: #0[#W%d#0]#n\n\r", pArea->security );
    send_to_char( buf, ch );

    sprintf( buf, "#wBuilders: #0[#W%s#0]#n\n\r", pArea->builders );
    send_to_char( buf, ch );

  sprintf(buf, "#wMusic   : #0[#W%s#0]#n\n\r", pArea->music);
  send_to_char(buf, ch);

    sprintf( buf, "#wFlags:    #0[#W%s#0]#n\n\r", flag_string( area_flags, pArea->area_flags ) );
    send_to_char( buf, ch );

  sprintf(buf, "#wBits:     #0[#W%s#0]#n\n\r", flag_string(area_bits, pArea->areabits));
  send_to_char(buf, ch);
...
bool aedit_vnum( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "AEdit:  Lower vnum %d already assigned in area %s.\n\r",ilower, pArea2->name);
	send_to_char( buf, ch );
	return FALSE;
    }
...
bool redit_show( CHAR_DATA *ch, char *argument )
{
  ...
  char buf2[MAX_STRING_LENGTH];
    ...
    sprintf( buf, "#wName:       #0[#W%s#0]\n\r#wArea:       #0[#W%5d#0] %s\n\r",
	    pRoom->name, pRoom->area->vnum, pRoom->area->name );
    strcat( buf1, buf );

    sprintf( buf, "#wVnum:       #0[#W%5d#0]\n\r#wSector:     #0[#W%s#0]\n\r",
	    pRoom->vnum, flag_string( sector_flags, pRoom->sector_type ) );
    strcat( buf1, buf );

    sprintf( buf, "#wRoom flags: #0[#W%s#0]\n\r",
  ...
  if (pRoom->roomtext)
  {
	...
	if ( ( pexit = pRoom->exit[door] ) )
	{
	    ...
	    sprintf( buf, "#R-#Y%-5s #wto #0[#W%5d#0] #wKey: #0[#R%5d#0]#n",
	    ...
	    {
		sprintf( buf, "#wKwds: #0[#W%s#0]#n\n\r", pexit->keyword );
		strcat( buf1, buf );
	    }
	    if ( pexit->description && pexit->description[0] != '\0' )
	    {
		sprintf( buf, "#B%s#n", pexit->description );
		strcat( buf1, buf );
	    }
	}
    }
...
bool oedit_show( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "#wVnum:        #0[#W%5d#0]#n\n\r#wType:        #0[#W%s#0]#n\n\r",
	pObj->vnum,
	flag_string( type_flags, pObj->item_type ) );
    send_to_char( buf, ch );

    sprintf( buf, "#wWear flags:  #0[#W%s#0]#n\n\r",
	flag_string( wear_flags, pObj->wear_flags ) );
    send_to_char( buf, ch );

    sprintf( buf, "#wExtra flags: #0[#W%s#0]#n\n\r",
	flag_string( extra_flags, pObj->extra_flags ) );
    send_to_char( buf, ch );
    
    sprintf( buf, "#wQuest flags: #0[#W%s#0]#n\n\r",
	flag_string( quest_flags, pObj->quest ) );
    send_to_char( buf, ch );
    
    sprintf( buf, "#wWeight:      #0[#W%d#0]#n\n\r#wCost:        #0[#W%d#0]#n\n\r",
	pObj->weight, pObj->cost );
    send_to_char( buf, ch );
    
    sprintf( buf, "#wCondition:   #0[#W%d#0]#n\n\r#wToughness:   #0[#W%d#0]#n\n\r",
	pObj->condition, pObj->toughness );
    send_to_char( buf, ch );
    
    sprintf( buf, "#wResistance:  #0[#W%d#0]#n\n\r",
	pObj->resistance );
    send_to_char( buf, ch );
	...
	sprintf( buf, "#0[#Y%4d#0] #w%-8d #B%s#n\n\r", cnt,
	    paf->modifier,
...
bool oedit_delete( CHAR_DATA *ch, char *argument )
{
     ...
     sprintf( buf, "Removed object vnum #C%d#n and #Y%d#n resets.\n\r", index,count );
     send_to_char( buf, ch );
     return TRUE;
}
...
bool medit_show( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "#wAct:         #0[#W%s#0]\n\r",
	flag_string( act_flags, pMob->act ) );
    send_to_char( buf, ch );

    sprintf( buf, "#wVnum:        #0[#R%5d#0]\n\r#wSex:         #0[#W%s#0]#n\n\r",
	pMob->vnum,
    ...
    //sprintf( buf, "#wBones:       #0[#W%d#0]\n\r",
    //pMob->bones );
    //send_to_char( buf, ch );

    sprintf( buf, "#wAffected by: #0[#W%s#0]\n\r",
	flag_string( affect_flags, pMob->affected_by ) );
    send_to_char( buf, ch );

    sprintf( buf, "#wDefault pos: #0[#W%s#0]#n\n\r",
	position_table[pMob->default_pos].name );
    send_to_char( buf, ch );

    sprintf( buf, "#wv0: Toughness         #0[#W%4d#0]\n\r",pMob->toughness);
    stc( buf, ch);
    sprintf( buf, "#wv1: Damage Modifier   #0[#W%4d#0]\n\r",pMob->dam_modifier);
    stc(buf,ch);
    sprintf( buf, "#wv2: Extra Attacks     #0[#W%4d#0]\n\r",pMob->extra_attack);
    stc(buf,ch); 

  switch (pMob->natural_attack)
  {
    default:
      sprintf(buf, "#wAttack     : #0[#RHit#0]\n\r");
      break;
    case 1:
      sprintf(buf, "#wAttack     : #0[#RSlice#0]\n\r");
      break;
    case 2:
      sprintf(buf, "#wAttack     : #0[#RStab#0]\n\r");
      break;
    case 3:
      sprintf(buf, "#wAttack     : #0[#RSlash#0]\n\r");
      break;
    case 4:
      sprintf(buf, "#wAttack     : #0[#RWhip#0]\n\r");
      break;
    case 5:
      sprintf(buf, "#wAttack     : #0[#RClaw#0]\n\r");
      break;
    case 6:
      sprintf(buf, "#wAttack     : #0[#RBlast#0]\n\r");
      break;
    case 7:
      sprintf(buf, "#wAttack     : #0[#RPound#0]\n\r");
      break;
    case 8:
      sprintf(buf, "#wAttack     : #0[#RCrush#0]\n\r");
      break;
    case 9:
      sprintf(buf, "#wAttack     : #0[#RGrep#0]\n\r");
      break;
    case 10:
      sprintf(buf, "#wAttack     : #0[#RBite#0]\n\r");
      break;
    case 11:
      sprintf(buf, "#wAttack     : #0[#RPierce#0]\n\r");
      break;
    case 12:
      sprintf(buf, "#wAttack     : #0[#RSuck#0]\n\r");
      break;
  }
  send_to_char(buf, ch);
	...
	for (cnt=0, list=pMob->mprogs; list; list=list->next)
	{
		...
		sprintf(buf, "#0[#W%5d#0] #R%5d  #B%7s #G%s#n\n\r", cnt,
			list->vnum,prog_type_to_name(list->trig_type),
			list->trig_phrase);
		send_to_char( buf, ch );
		cnt++;
    ...
    if (pMob->quest_fun)
  {
    sprintf(buf, "#wQuest fun:    #0[#W%s#0]\n\r", quest_string(pMob->quest_fun));
    send_to_char(buf, ch);
  }
if (pMob->shop_fun)
  {
    sprintf(buf, "#wShop fun:    #0[#W%s#0]\n\r", shop_string(pMob->shop_fun));
    send_to_char(buf, ch);
  }
    sprintf( buf, "#wShort descr: #n%s\n\r#wLong descr:\n\r#n%s",
	pMob->short_descr,
	pMob->long_descr );
    send_to_char( buf, ch );
