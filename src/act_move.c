...
void do_disciplines(CHAR_DATA *ch, char *argument)    
{
    ...
    if (IS_CLASS(ch, CLASS_WEREWOLF))
        sprintf(buf, "#0---===#W[ #RGuardians #W]#0===---#n");
    else
        sprintf(buf, "#0---===#W[ #RDisciplines #W]#0===---#n");
...
return;
  ...
  if (!str_cmp(arg1,"purple"))
    ...
    if (is_number(arg2))
    {
      ...
      {
      	  sprintf(buf, "Please enter a value between 1 and %d.\n\r", cmax - ch->spl[0]);
          send_to_char(buf,ch);
          return;
      }
    }
    ...
    {
      sprintf( buf, "You gain %d points of purple spell level.\n\r", urin_counter );
      send_to_char( buf, ch );
    }	
return;
    }
    ...
    {
      sprintf( buf, "You gain %d points of blue spell level.\n\r", urin_counter );
      send_to_char( buf, ch );
    }	
return;
    }
    ...
    {
      sprintf( buf, "You gain %d points of red spell level.\n\r", urin_counter );
      send_to_char( buf, ch );
    }	
return;
    }
    ...
    {
      sprintf( buf, "You gain %d points of green spell level.\n\r", urin_counter );
      send_to_char( buf, ch );
    }	
return;
    }
    ...
    {
      sprintf( buf, "You gain %d points of yellow spell level.\n\r", urin_counter );
      send_to_char( buf, ch );
    }	
return;
    }
...
return;
    ...
    if ( arg1[0] == '\0' )
    {
	sprintf( buf, "#0You have #Y%d #0experience points and #Y%d #0primal.#n\n\r", ch->exp, ch->practice );
	send_to_char( buf, ch );
	sprintf(arg1,"foo");
    }
...
&& (ch->pcdata->rank == BELT_NINE))
  {
		...
		else 
	{
		...
		sprintf(buf,"#R%s #Chas just become a more #r-#R=P#YOW#7ER#YFU#RL=#r- #cG#Cod!!#n\n\r",ch->name );
		do_info(ch,buf);
		return;
	}
	}
//Next One below
    ...
    {
	sprintf( buf, "#0You can train the following#w:#n\n\r" );
	send_to_char( buf, ch );

	send_to_char( "#0Stats#w:#W", ch );
	if ( ch->pcdata->perm_str < max_stat ) 
	{
		sprintf( buf, " #WStr#B(#C%dp#B)", figure_stat_cost(ch->pcdata->perm_str) );
		stc( buf, ch );
	}
	if ( ch->pcdata->perm_int < max_stat ) 
	{
		sprintf( buf, " #WInt#B(#C%dp#B)", figure_stat_cost(ch->pcdata->perm_int) );
		stc( buf, ch );
	}
	if ( ch->pcdata->perm_wis < max_stat ) 
	{
		sprintf( buf, " #WWis#B(#C%dp#B)", figure_stat_cost(ch->pcdata->perm_wis) );
		stc( buf, ch );
	}
	if ( ch->pcdata->perm_dex < max_stat ) 
	{
		sprintf( buf, " #WDex#B(#C%dp#B)", figure_stat_cost(ch->pcdata->perm_dex) );
		stc( buf, ch );
	}
	if ( ch->pcdata->perm_con < max_stat ) 
	{
		sprintf( buf, " #WCon#B(#C%dp#B)", figure_stat_cost(ch->pcdata->perm_con) );
		stc( buf, ch );
	}
	if ( ( ch->pcdata->perm_str >= max_stat )
	...
	if ( ch->tier == 0)
	{
		...
		sprintf( buf, "#RTier I: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
	...
	if ( ch->tier == 1)
	{
		...
		sprintf( buf, "#RTier II: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
		...
		if ( ch->tier == 2)
	{
		...
		sprintf( buf, "#RTier III: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
	...
	if ( ch->tier == 3)
	{
		...
		sprintf( buf, "#RTier IV: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
	...
	if ( ch->tier == 4)
	{
		...
		sprintf( buf, "#RTier V: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
	...
	if ( ch->tier == 5)
	{
		...
		sprintf( buf, "#RTier VI: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
		...
		if ( ch->tier == 6)
	{
		...
		sprintf( buf, "#RTier VII: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
	...
	if ( ch->tier == 7)
	{
		...
		sprintf( buf, "#RTier VIII: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
	...
	if ( ch->tier == 8)
	{
		...
		sprintf( buf, "#RTier IX: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
	...
	if ( ch->tier == 9)
	{
		...
		sprintf( buf, "#RTier X: #DYou need #Y%d #Dmore Tier Kills, #Y%d #Dmore Status, and #Y%d #Dmore HP.#n\n\r",
		TKsNeeded,
		StatusNeeded,
		HitNeeded
		);
	...
	{
	   sprintf( buf, "#GBecome an avatar -  free.#n\n\r" );
	   send_to_char( buf, ch );
	}
	...
	{
	   sprintf( buf, "#WSilver tolerance #R- #Y%d #0exp per point of tolerance.#n\n\r", silver );
	   send_to_char( buf, ch );
	}
       if ( ch->gnosis[GMAXIMUM] < 20 && (IS_CLASS(ch, CLASS_WEREWOLF)))
       {
	   sprintf( buf, "#WGnosis Points #R- #Y%d #0exp per point of Gnosis.#n\n\r", gnosis );
	   stc( buf, ch );
       }
       if ( ch->beast > 0 && IS_CLASS(ch, CLASS_VAMPIRE) )
       {
	   sprintf( buf, "#WBeast Control #R- #Y%d #0primal per point of Beast.#n\n\r", beast );
	   stc( buf, ch );
       }
	if (ch->pcdata->stats[DROW_MAGIC] < 100 && IS_CLASS(ch, CLASS_DROW))
	{ sprintf( buf, "#PMagic resistance #R- #Y%d #pdrow points per point of magic.#n\n\r", magic );
	  send_to_char( buf, ch);}
	
	sprintf( buf, "#WNatural resistances and immunities #R- #Y%d #0exp each.#n\n\r", immcost );
	send_to_char( buf, ch );
...
void do_tie( CHAR_DATA *ch, char *argument )
{
  ...
  if (IS_NPC(ch))
    ...
    if (victim->in_room != NULL)
    {
      ...
      {
        sprintf(buf,"#C%s #yhas been vanquished from the #Rarena#y by #C%s#n",victim->name,ch->name);
        do_info(ch,buf);
      ...
      if ((location = get_room_index(ROOM_VNUM_ALTAR)) == NULL)
        ...
        if (!found)
        {
          sprintf(buf,"#C%s #yemerges victorious from the #Rarena#n",ch->name);
          ch->pcdata->awins++;
          do_info(ch,buf);
