...
char *commanum( int number )
{
  ...
  sprintf(thing, "%d", number);
  strcpy(string, "");

  for ( x = (strlen(thing) - 1), y = 0; x >= 0; x--)
  {
    sprintf(string2, "%s%s%c", string, (y >=3) ? "," : "", thing[x]);
    if (y >= 3)
      y = 0;
    strcpy(string, string2);
    sprintf(negative, "%c", thing[x]);
    if (str_cmp(negative, "-"))
    y++;
  }
  strcpy(thing, string);
  ...
  {
    sprintf(string2, "%s%c", string, thing[x]);
    strcpy(string, string2);
  }
  return &string[0];
}
	...
	if ( !str_cmp( arg1, "/lr" ) )
	{
		...
		*ch->desc->pString = string_lineadd( *ch->desc->pString, tmparg3, atoi(arg2) );
        ...
        if ( !str_cmp( arg1, "/h" ) )
        {
            ...
            send_to_char( "/li <num> <str>  - add <str> before line <num>\n\r", ch );
	    	send_to_char( "/lr <num> <str>  - replace line <num> for <str>\n\r", ch );
            send_to_char( "@                - end string          \n\r", ch );
            return;
        }
            

        send_to_char( "Editor:  Invalid command.\n\r", ch );
        return;
    }
