...
void do_quest( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg2[0] == '\0' )
	{
	    ...
	    return;
    	     ...
    	     if (!str_cmp(arg2,"light"    )) {add = ITEM_LIGHT;     value = 1000;}
    	else if (!str_cmp(arg2,"weapon"   )) {add = ITEM_WEAPON;    value = 5000;}
    	else if (!str_cmp(arg2,"armor"    )) {add = ITEM_ARMOR;     value = 2000;}
    	else if (!str_cmp(arg2,"armour"   )) {add = ITEM_ARMOR;     value = 2000;}
    	else if (!str_cmp(arg2,"container")) {add = ITEM_CONTAINER; value = 1000;}
    	else if (!str_cmp(arg2,"boat"     )) {add = ITEM_BOAT;      value = 1000;}
    	else if (!str_cmp(arg2,"fountain" )) {add = ITEM_FOUNTAIN;  value = 1000;}
    	else if (!str_cmp(arg2,"stake"    )) {add = ITEM_STAKE;     value = 1000;}
	else
	{
 ...
 "Armor (30 Bones),\n\rContainer (10 Bones), Boat (10 Bones), Fountain <type> (10 Bones), Stake (10 Bones).\n\r",ch);
	...
	if (ch->bones < value)
	{
	    sprintf(buf,"You don't have the required %d bones.\n\r",value);
	    send_to_char(buf,ch);
	    return;
	}
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
	send_to_char( "#0- - - - - - - - - - #w----====#W[ #BQUEST ITEM COSTS #W]#w====---- #0- - - - - - - - - -#n\n\r",	 ch );
	send_to_char( "Create: Creates a new personalized object, costing between 1000 and 5000 Bones.\n\r",ch );
	send_to_char( "Name/Short/Long: Rename the object.  100 Bones for all three.\n\r",ch );
	send_to_char( "Protection: Sets AC on armour at 100 Bones per point.\n\r", ch );
	send_to_char( "Min/Max: Sets min/max damage on weapon at 100 Bones per point.\n\r", ch );
	send_to_char( "Weapon: Sets weapon type for 1000 Bones.\n\r", ch );
	send_to_char( "Extra (add/remove): Glow(100/100), Hum(100/100), Invis(100/100), Anti-Good(100/1000),"
                    "Anti-Neutral(100/1000), Anti-Evil(100/1000), Loyal(1000/100).\n\r",ch );
	send_to_char( "Wear: Select location, costs 2000 Bones's.  Type 'quest <obj> wear' to see locations.\n\r",ch );
	send_to_char( "Power: Spell power for spell weapons.  Costs 100 Bones per power point.\n\r",ch );
	send_to_char( "Spell: Spell weapons or affect.  Costs 5000 Bones.\n\r",ch );
	send_to_char( "Transporter: Future transportation to that room.  Costs 5000 Bones.\n\r",ch );
	...
	send_to_char( "Weight: Set objects weight to 1.  Costs 1000 Bones.\n\r",ch );
	send_to_char( "Str, Dex, Int, Wis, Con... max =   3 each, at  2000 bones per +1 stat.\n\r", ch);
	send_to_char( "Hp, Mana, Move............ max =  25 each, at   500 bones per point.\n\r", ch);
	send_to_char( "Hitroll, Damroll.......... max =   5 each, at  3000 bones per point.\n\r", ch);
	send_to_char( "Ac........................ max = -25,      at  1000 points per point.\n\r", ch);
	send_to_char( "#0- - - - - - - - - - #w----====#W[ #BQUEST ITEM COSTS #W]#w====---- #0- - - - - - - - - -#n\n\r",	 ch );
	return;
    }
    ...
    if (obj->item_type == ITEM_QUEST || obj->item_type == ITEM_AMMO ||
        ...
        obj->item_type == ITEM_TOOL  || obj->item_type == ITEM_SYMBOL || obj->item_type == ITEM_FAITHTOKEN ||
	...
	if (IS_SET(obj->quest,QUEST_RELIC))
	    ...
	    if(( !str_cmp(arg3,"bite") || !str_cmp(arg3,"grep")) && ch->bones < 10000)
	    {
	...
	if      ( !str_cmp( arg3, "glow"         )) {value = ITEM_GLOW; add = 100; remove = 100;}
	else if ( !str_cmp( arg3, "hum"          )) {value = ITEM_HUM; add = 100; remove = 100;}
	else if ( !str_cmp( arg3, "invis"        )) {value = ITEM_INVIS; add = 100; remove = 100;}
	else if ( !str_cmp( arg3, "anti-good"    )) {value = ITEM_ANTI_GOOD; add = 100; remove = 1000;}
	else if ( !str_cmp( arg3, "anti-evil"    )) {value = ITEM_ANTI_EVIL; add = 100; remove = 1000;}
	else if ( !str_cmp( arg3, "anti-neutral" )) {value = ITEM_ANTI_NEUTRAL; add = 100; remove = 1000;}
	else if ( !str_cmp( arg3, "loyal"        )) {value = ITEM_LOYAL; add = 1000; remove = 100;}
	else if ( !str_cmp( arg3, "silver"       )) {value = SITEM_SILVER; add = 10000; remove = 0;}
	else
	{
	    send_to_char("Enter one of: Glow, Hum, Invis, Anti-good, Anti-evil, Anti-neutral, Loyal,\n\rSilver.\n\r",ch);
	    return;
	...
	if (!str_cmp( arg3, "silver" ))
	{
	    ...
	    {
		sprintf(buf,"Sorry, you need %d bones to set that flag.\n\r",add);
...

	{
		...
		sprintf(buf,"Sorry, you need %d bones to remove that flag.\n\r",remove);
...
{
	    send_to_char("It costs 50 bones to create a spell weapon or affect.\n\r", ch);
	    return;
	}
	if (weapon > 0)
	{
	...
	{
	    send_to_char("It costs 5000 bones to create a spell weapon or affect.\n\r", ch);
	    return;
	}
	if (weapon > 0)
	{
	...
	{
	    send_to_char("You don't have enough bones to increase the spell power.\n\r",ch);
	    return;
	}
	...
	{
	    send_to_char("It costs 1000 quest point to remove the weight of an object.\n\r",ch);
	    return;
	}
	obj->weight = 1;
	ch->bones -= 1000;
	...
	{
	    send_to_char("It costs 5000 quest point to create a transporter.\n\r",ch);
	    return;
	}
        ...
        if (IS_SET(obj->quest, QUEST_RELIC) && !IS_SET(obj->quest, QUEST_HOLYSYMBOL))
{
	...
	if (ch->bones < value)
	{
	    send_to_char("It costs 100 bones to rename an object.\n\r",ch);
	    return;
	}
...
void oset_affect( CHAR_DATA *ch, OBJ_DATA *obj, int value, int affect, bool is_quest )
{
    ...
    if      ( affect == APPLY_STR     ) {range = 3;  cost = 2000 ; quest = QUEST_STR;}
    else if ( affect == APPLY_DEX     ) {range = 3;  cost = 2000 ; quest = QUEST_DEX;}
    else if ( affect == APPLY_INT     ) {range = 3;  cost = 2000 ; quest = QUEST_INT;}
    else if ( affect == APPLY_WIS     ) {range = 3;  cost = 2000 ; quest = QUEST_WIS;}
    else if ( affect == APPLY_CON     ) {range = 3;  cost = 2000 ; quest = QUEST_CON;}
    else if ( affect == APPLY_HIT     ) {range = 25; cost =  500 ; quest = QUEST_HIT;}
    else if ( affect == APPLY_MANA    ) {range = 25; cost =  500 ; quest = QUEST_MANA;}
    else if ( affect == APPLY_MOVE    ) {range = 25; cost =  500 ; quest = QUEST_MOVE;}
    else if ( affect == APPLY_HITROLL ) {range = 5;  cost = 3000 ; quest = QUEST_HITROLL;}
    else if ( affect == APPLY_DAMROLL ) {range = 5;  cost = 3000 ; quest = QUEST_DAMROLL;}
    else if ( affect == APPLY_AC      ) {range = 25; cost = 1000 ; quest = QUEST_AC;}
    else return;
	...
	send_to_char("Str, Dex, Int, Wis, Con... max =   3 each, at  2000 bones per +1 stat.\n\r", ch);
	send_to_char("Hp, Mana, Move............ max =  25 each, at   500 quest point per point.\n\r", ch);
	send_to_char("Hitroll, Damroll.......... max =   5 each, at  3000 bones per point.\n\r", ch);
	send_to_char("Ac........................ max = -25,      at  1000 points per point.\n\r", ch);
    ...
    {
	sprintf(buf,"You are limited to %d bones per item.\n\r",max);
	send_to_char(buf, ch);
	return;
    }
    ...
    {
	sprintf(buf,"That costs %d bones, while you only have %d.\n\r",cost, ch->bones);
	send_to_char(buf, ch);
	return;
    }

    if (!IS_SET(obj->quest, quest) ) SET_BIT(obj->quest,quest);
    if (is_quest) ch->bones -= cost;
    obj->points += cost;
    if (obj->questmaker != NULL) free_string(obj->questmaker);
    obj->questmaker = str_dup(ch->name);
