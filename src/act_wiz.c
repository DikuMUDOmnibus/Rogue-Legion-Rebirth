...
void do_info( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf(buf, "#b-=#B[ #RInfo #B]#b=-#n #w%s#n\n\r", argument);
...
void do_ainfo( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf(buf, "#C@#D[#yxxx#D]#w(#W>>>#BArena#W>>#r>#R> #w%s#n\n\r", argument);
...
void do_hint( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf(buf, "#P<- #GHint #P->#n %s\n\r", argument);

    for ( d = descriptor_list; d != NULL; d = d->next )
    {
	if ( d->connected == CON_PLAYING && d->character != NULL && !IS_SET(d->character->deaf, CHANNEL_HINT) )
	{
      send_to_char(buf, d->character);
	}
    }
...
void do_ostat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Other Quest Bits: %s.\n\r",
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf(buf, "Lv: %d.  Align: %d.  AC: %d.  Exp: %d.\n\r", victim->level, victim->alignment, char_ac(victim), victim->exp);
    ...
    sprintf( buf, "Generation: %d.  Genexp %d.\n\r", victim->generation, victim->genexp);
    stc(buf, ch);
...
void do_omni(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf," Name         |Level|Trust|Gen|  Hit | Mana | Move |  HR|  DR|   AC| Bones\n\r");
	send_to_char(buf,ch);
        sprintf(buf,"--------------|-----|-----|---|------|------|------|----|----|-----|--------\n\r");
	send_to_char(buf,ch);
...
sprintf(buf,"%-14s|%5d|%5d|%3d|%6d|%6d|%6d|%4d|%4d|%5d|%8d\n\r",
...
void do_users( CHAR_DATA *ch, char *argument )
{
	...
	{
	    sprintf( buf + strlen(buf), "#Y[#w%3d %16s#Y]  #R%s#G@#Y%s\n\r",
		d->descriptor,
		st,
...
void do_artifact( CHAR_DATA *ch, char *argument )
{
  ...
  if (IS_NPC(ch))
  {
    ...
    if( IS_IMMORTAL(ch) )
        {
            sprintf( buf, "%5d | %20.20s | %-10.10s#n | %s\n\r",
        ...
        {
            sprintf( buf, "%s, #0which has been discovered by #B%s#n.\n\r",
...
void do_token( CHAR_DATA *ch, char *argument )
{
	...
	if ( value < 1 || value > 100000 )
        ...
        {
	    sprintf(buf,"You only have %d bones left to put into tokens.\n\r",ch->bones);
	    send_to_char( buf, ch );
	    return;
        }
    }
...
void do_toggle( CHAR_DATA *ch, char *argument )
{
     ...
     if (!str_cmp(arg2, "off"))
  {
    ...
    global_qp = TRUE;
        sprintf( buf, "%s has declared double bones for everyone \n\r", ch->name );
    do_info( ch, buf );
    ...
    if (!str_cmp(arg2, "off"))
    {
        ...
        global_qp = FALSE; 
        sprintf(buf, "%s has removed double bones!\n\r", ch->name );
	do_info( ch, buf );
  ...
  return;
    ...
    global_dt = TRUE;
        sprintf( buf, "#R#fSuper Training#n #wNOW in affect!\n\r");
    do_info( ch, buf );
    ...
    if (!str_cmp(arg2, "off"))
    {
        ...
        global_dt = FALSE; 
        sprintf(buf, "#RSUPER TRAINING #wmode off!\n\r");
	do_info( ch, buf );
...
if (!str_cmp(arg, "cp"))
    ...
    global_cp = TRUE;
        sprintf( buf, "%s has declared double Class Points for everyone \n\r", ch->name );
    do_info( ch, buf );
    return;
  }
    if (!str_cmp(arg2, "off"))
    {
        ...
        global_cp = FALSE; 
        sprintf(buf, "%s has removed double class points!\n\r", ch->name );
	do_info( ch, buf );
  return;
    }}
   ...
   if (!str_cmp(arg, "chaos"))
    ...
    global_chaos = TRUE;
        sprintf( buf, "%s throws the world into #sC#sH#sA#sO#sS#n\n\r", ch->name );
    do_info( ch, buf );
    return;
  }
    if (!str_cmp(arg2, "off"))
    {
        ...
        global_chaos = FALSE; 
        sprintf(buf, "%s restores #YOrder#n to the world#n\n\r", ch->name );
	do_info( ch, buf );
  return;
    }}
    ...
    global_exp = TRUE;
    global_dt = TRUE;
        sprintf( buf, "%s has turned on Double Bones, Class Points, EXP, and Training  \n\r", ch->name );
    do_info( ch, buf );
    ...
    global_exp = FALSE;
    global_dt = FALSE;
        sprintf(buf, "%s has turned off all the doubles!\n\r", ch->name );
	do_info( ch, buf );
