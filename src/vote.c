...
"#D -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n\r"
"#w #PSyntax#D: #Cvote yes #D- #Cvote no#n\n\r",
			ch->name,
			argument,
			atoi(arg2) );
...
void vote_update( void )
{
	...
	if( vote[0] <= 0 )
	{
		...
		sprintf(buf,"#wVote has ended for topic #D[#W%s#D]#n\n\r", vote_topic );
                do_info(NULL, buf);
                sprintf(buf,"#wVote outcome is #GYes#D[#Y%.2f%%#D] #RNo#D[#Y%.2f%%#D] #BTotal Votes#D[#Y%d#D]#n\n\r",
				x1,
				x2,
				(int)total );
                do_info(NULL,buf);
