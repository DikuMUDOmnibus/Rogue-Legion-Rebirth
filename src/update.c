...
if (IS_CLASS(ch, CLASS_DROID))
        ...
        if (IS_CLASS(ch,CLASS_SHINOBI)){ update_shinobi(ch);
...
void weather_update( void )
{
  ...
  switch ( ++time_info.hour )
  {
	...
	for ( d = descriptor_list; d != NULL; d = d->next )
	{
		...
		send_to_char( "#cYou hear a #Rclock #cin the #Rdistance #cstrike #0M#7i#0d#7n#0i#7g#0h#7t#n.\n\r",ch);
    ...
    for ( paf2 = ch->affected2; paf2 != NULL; paf2 = paf2_next )
    {
      ...
      else
      {
        if ( paf2_next == NULL  || paf2_next->type != paf2->type || paf2_next->duration > 0 )
        {
          ...
          {
            sprintf(buf, "Update : Bad affect2 on %s.", ch->name);
            bug(buf, 0);
          }
        }
    ...
    if (obj->item_type == ITEM_GRENADE && (locate_obj(obj))->people)
    {
      ...
      for (vch = (locate_obj(obj))->people; vch != NULL; vch = vch->next_in_room)
      {
        ...
        else
        {
          ...
          if (!bugObj)
          {
          	//sprintf(buf,"obj_update: obj %d in room %d not in object_list. Terminating obj_update.", obj->pIndexData->vnum, obj->in_room->vnum);
...
void hint_update( void )
{
  char buf[MSL];
      ...
      switch(number_range(0,19))
        {
        default: sprintf(buf, "To turn of hints type CHANNELS -HINT"); break;
        case 0: sprintf(buf, "Use CONFIG to change options"); break;
        case 1: sprintf(buf, "use HELP to find a help topic when in doubt"); break;
        case 2: sprintf(buf, "To get exp after newbie hours you must accept policy HELP POLICY"); break;
        case 3: sprintf(buf, "type CHANNELS to set communication channels"); break;
        case 4: sprintf(buf, "Feel free to post suggestions to the boards"); break;
        case 5: sprintf(buf, "type HISTORY to see a history of tells and replys"); break;
        case 6: sprintf(buf, "use AUCTION to make some extra money."); break;
        case 7: sprintf(buf, "To turn of hints type CHAN -HINT"); break;
        case 8: sprintf(buf, "Don't ask for restores or double exp/qp ;)"); break;
        case 9: sprintf(buf, "Don't eat #YYellow#n #WSnow#n"); break;
        case 10: sprintf(buf, "look at '#BHELP TIER#n' for tier information"); break;
        case 11: sprintf(buf, "make sure you have read HELP POLICY"); break;
        case 12: sprintf(buf, "Tell your friends to stop by"); break;
        case 13: sprintf(buf, "Type AUTOALL to set all your autos at once"); break;
        case 14: sprintf(buf, "Type #RAUTO#n to see your auto settings."); break;
        case 15: sprintf(buf, "Type #RCRECALL#n to go to your racial hideaway and buy class items."); break;
	case 16: sprintf(buf, "Type Gainsymbiote to get a life or death Symbiote."); break;
	case 17: sprintf(buf, "Type symbioteadvance to view your Symbiote screen."); break;
	case 18: sprintf(buf, "Type consecrate (weaponname) to advance your weapon."); break;
	case 19: sprintf(buf, "You can craft your own armor with armorcraft, (help armorcraft)"); break;
        }
      hint_channel(buf);
}
