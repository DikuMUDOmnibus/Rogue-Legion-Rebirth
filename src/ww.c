...
void do_tribe(CHAR_DATA *ch, char *argument)
{
  ...
  send_to_char("#w[#CName                #w] [#C  Hp   #w] [#C Mana  #w] [#C Move  #w]#n\n\r",ch);
  for ( gch = char_list; gch != NULL; gch = gch->next)
  {
    if (IS_NPC(gch)) continue;
    if (!IS_CLASS(gch, CLASS_WEREWOLF)) continue;
    sprintf( buf,"#w[%-20s] [%-7d] [%-7d] [%-7d]#n\n\r",
...
void do_gklaive( CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_CLASS(ch, CLASS_WEREWOLF))
  {
    ...
    {
	send_to_char("You need to obtain level 7 in Luna to make a Greater Klaive.\n\r",ch);
    	return;
    }
  
  if ( 200 > ch->practice)
  {
    send_to_char("It costs 200 points of primal to create a Greater Klaive.\n\r",ch);
    return;
  }
  ch->practice   -= 200;
  ...
  SET_BIT(obj->spectype, SITEM_WOLFWEAPON);
  obj_to_char(obj, ch);
  act("A Greater Klaive appears in your hands in a flash of light.",ch,NULL,NULL,TO_CHAR);
  act("A Greater Klaive appears in $n's hands in a flash of light.",ch,NULL,NULL,TO_ROOM);
  return;
}
...
void do_moonarmour( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->practice < 100)
    {
	send_to_char("It costs 100 primal to create a piece of moon armour\n\r",ch);
	return;
    }
