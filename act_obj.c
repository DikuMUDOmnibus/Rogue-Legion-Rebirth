...
send_to_char( "#rYou force down yer beast#n.\n\r",ch);
	...
	!IS_VAMPAFF(ch,VAM_DISGUISED) && !IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_FAE) &&
        !IS_CLASS(ch, CLASS_DEMON) && ch->power[DISC_VAMP_OBEA] < 9 && !IS_CLASS(ch, CLASS_CYBORG) && !IS_CLASS(ch, CLASS_DROID))
    {
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_cmp(arg, "all") || !str_prefix("all." ,arg))
    {
      ...
      for (obj = ch->in_room->contents; next_obj != NULL; obj = next_obj )
      {
        ...
        {
            sprintf( buf, "You receive a refund of %d bones from $p.", obj->points);
        ...
        {
        default:
        sprintf(buf, "#0The Gods appreciate your sacrifice of #w$p#n");
        act( buf, ch, obj, NULL, TO_CHAR );
        break;
        case 2:
        sprintf(buf, "#0You get #Y%d #0exp bonus for the sacrifice of #w$p#n", ereward);
        act( buf, ch, obj, NULL, TO_CHAR );
        ch->exp += ereward;
        break;
        case 3:
        sprintf(buf, "#0The gods are #RDISPLEASED #0with the offering of #n$p and #RDOCK#0 you #Y%d #0exp#n", ereward / 2);
        act( buf, ch, obj, NULL, TO_CHAR );
        ch->exp -= ereward / 2 ;
        if(ch->exp < 0)
        {
        ...
        break;
        case 1:
        sprintf(buf, "#RThe Gods are especially pleased with #0$p #Rand give you #Y5000000 #Rexp#n");
        act( buf, ch, obj, NULL, TO_CHAR );
                    ch->exp += 5000000;
       		    break;
		}
...
{
	sprintf(buf, "$p disintegrates into a fine powder");
        act( buf, ch, obj, NULL, TO_CHAR );
}
...
//		log_string( buf );
	...
	if(victim->bones < 0)
	victim->bones = 0;
	sprintf( buf, "Bingo!  You got %d bones.\n\r", amount );
	send_to_char( buf, ch );
	do_autosave(ch,"");
	do_autosave(victim,"");
	return;
...
void do_claim( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_SET(obj->quest, QUEST_ARTIFACT))
	{
	send_to_char("You can not claim artifacts.\n\r", ch);
	return;
	}    
